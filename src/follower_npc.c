...
u32 DetermineFollowerNPCState(struct ObjectEvent *follower, u32 state, enum Direction direction)
{
    ...
    if (IsPlayerForcedOntoSameTile(newPlayerMB, playerMoveDirection)
     && !((TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_BIKE) && gSaveBlock3Ptr->playerBike == ACRO_BIKE) && playerMoveDirection == DIR_NORTH && newPlayerMB == MB_MUDDY_SLOPE && GetPlayerSpeed() >= PLAYER_SPEED_FAST))
    {
    ...
    if (IsStateMovement(state) && delayedState)
    {
        ...
        if (delayedState == MOVEMENT_ACTION_JUMP_DOWN
         && (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_BIKE) && gSaveBlock3Ptr->playerBike == ACRO_BIKE))
...
void SetFollowerNPCSprite(u32 spriteIndex)
{
    ...
    clone = *GetObjectEventTemplateByLocalIdAndMap(LOCALID_NPC_FOLLOWER, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup);
