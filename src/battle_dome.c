...
static int GetTypeEffectivenessPoints(enum Move move, enum Species targetSpecies, int mode)
{
    ...
    if ((defAbility == ABILITY_LEVITATE || defAbility == ABILITY_EELEVATE) && moveType == TYPE_EARTH)
    {
...
static void DisplayTrainerInfoOnCard(u8 flags, u8 trainerTourneyId)
{
    ...
    u32 trainerPic = GetPlayerTrainerPicIdByOutfitGenderType(gSaveBlock3Ptr->currOutfitId, gSaveBlock2Ptr->playerGender);
    ...
    i = 0;
    if (trainerId == TRAINER_PLAYER)
        j = gFacilityClassToTrainerClass[FACILITY_CLASS_RENKO];
    else if (trainerId == TRAINER_FRONTIER_BRAIN)
...
static void DisplayMatchInfoOnCard(u8 flags, u8 matchNo)
{
    ...
    int x = 0, y = 0;
    u8 palSlot = 0;
    u16 picId = GetPlayerTrainerPicIdByOutfitGenderType(gSaveBlock3Ptr->currOutfitId, gSaveBlock2Ptr->playerGender);
