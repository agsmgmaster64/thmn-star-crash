...
static const u8 sRivalAvatarGfxIds[][GENDER_COUNT] =
{
    ...
    [PLAYER_AVATAR_STATE_UNDERWATER] = {OBJ_EVENT_GFX_RENKO_UNDERWATER,       OBJ_EVENT_GFX_MARIBEL_UNDERWATER},
};
...
static const u8 sRivalGfxIds[][GENDER_COUNT] =
{
    ...
    [PLAYER_AVATAR_GFX_FISHING]    = {OBJ_EVENT_GFX_RENKO_FISHING,          OBJ_EVENT_GFX_MARIBEL_FISHING},
    [PLAYER_AVATAR_GFX_WATERING]   = {OBJ_EVENT_GFX_RENKO_WATERING,         OBJ_EVENT_GFX_MARIBEL_WATERING},
...
static void PlayerNotOnBikeMoving(enum Direction direction, u16 heldKeys)
{
    ...
    {
        if (heldKeys & B_BUTTON && gSaveBlock3Ptr->autoRun == TRUE)
        {
...
{
    return GetPlayerAvatarGraphicsIdByOutfitStateIdAndGender(gSaveBlock3Ptr->currOutfitId, state, gender);
}
...
{
    EndORASDowsing();
    u16 gfxId = GetPlayerAnimGraphicsIdByOutfitStateIdAndGender(gSaveBlock3Ptr->currOutfitId, playerAnimId, gSaveBlock2Ptr->playerGender);
...
bool8 ObjectMovingOnRockStairs(struct ObjectEvent *objectEvent, enum Direction direction)
{
        ...
        if (IsFollowerVisible() && GetFollowerObject() != NULL && (objectEvent->isPlayer || objectEvent->localId == LOCALID_FOLLOWER))
            return FALSE;
...
{
    if (gPlayerAvatar.flags & (PLAYER_AVATAR_FLAG_BIKE))
        SetPlayerAvatarAnimation(PLAYER_AVATAR_GFX_VSSEEKER_BIKE, 0);
    else
