...
static void SetupOutfitMenu_PrintStr(void)
{
    PrintTexts(WIN_INFO, FONT_NORMAL, 2, 0, COLORID_NORMAL, gOutfits[gSaveBlock3Ptr->currOutfitId].name);
    PrintTexts(WIN_INFO, FONT_NORMAL, 2, 16, COLORID_NORMAL, gOutfits[gSaveBlock3Ptr->currOutfitId].desc);
...
static void SpriteCB_Indicator(struct Sprite *s)
{
    ...
    if (i == gSaveBlock3Ptr->currOutfitId)
    {
        s->invisible = FALSE;
    }
    else
...
static void SetupOutfitMenu_Grids(void)
{
    ...
    GridMenu_SetIndex(sOutfitMenu->grid, gSaveBlock3Ptr->currOutfitId - 1);
    sOutfitMenu->idx = sOutfitMenu->list[GridMenu_SelectedIndex(sOutfitMenu->grid)];
