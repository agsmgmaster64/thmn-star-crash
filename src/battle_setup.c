...
static void CB2_EndScriptedGhostBattle(void);
...
EWRAM_DATA u8 gNuzlockeIsCaptureBlocked = FALSE;
EWRAM_DATA u8 gNuzlockeIsSpeciesClauseActive = FALSE;
...
// works
static bool8 CheckIfGhostBattle(void)
{
    if (FlagGet(FLAG_GHOST_BATTLE) == TRUE
...
{
    CreateMaleMon(&gParties[B_TRAINER_OPPONENT_A][0], SPECIES_CHIBI_ELLEN, 5);
    LockPlayerFieldControls();
...
{
    CreateMaleMon(&gParties[B_TRAINER_OPPONENT_A][0], SPECIES_CHIBI_SHIZUHA, 5);
    LockPlayerFieldControls();
    gMain.savedCallback = CB2_ReturnToFieldContinueScriptPlayMapMusic;
    gBattleTypeFlags = BATTLE_TYPE_CATCH_TUTORIAL;
    CreateBattleStartTask(B_TRANSITION_SLICE, 0);
...
void StartScriptedGhostBattle(void)
{
    LockPlayerFieldControls();
    gMain.savedCallback = CB2_EndScriptedGhostBattle;
...
void StartMarowakBattle(void)
{
    ...
    if (CheckBagHasItem(ITEM_SILPH_SCOPE, 1))
    {
        u32 personality = GetMonPersonality(SPECIES_NORMAL_YUYUKO, MON_FEMALE, NATURE_SERIOUS, RANDOM_UNOWN_LETTER);

        CreateMonWithIVsPersonality(&gParties[B_TRAINER_OPPONENT_A][0], SPECIES_NORMAL_YUYUKO, 30, 31, personality);
    }
...
enum BattleEnvironments BattleSetup_GetEnvironmentId(void)
{
    ...
    if (MetatileBehavior_IsSnowGrass(tileBehavior))
        return BATTLE_ENVIRONMENT_SNOW_GRASS;
    if (MetatileBehavior_IsFallGrass(tileBehavior))
        return BATTLE_ENVIRONMENT_FALL_GRASS;
    ...
    case MAP_TYPE_UNDERGROUND:
        ...
        if ((gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_GRANITE_CAVE_1F) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_GRANITE_CAVE_1F))
         || (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_GRANITE_CAVE_B1F) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_GRANITE_CAVE_B1F))
         || (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_GRANITE_CAVE_B2F) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_GRANITE_CAVE_B2F))
         || (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_GRANITE_CAVE_STEVENS_ROOM) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_GRANITE_CAVE_STEVENS_ROOM)))
            return BATTLE_ENVIRONMENT_ICE;
        ...
        && TRAINER_BATTLE_PARAM.mode != TRAINER_BATTLE_CONTINUE_SCRIPT_DOUBLE_NO_MUSIC)
    {
        ...
        else
        {
            ...
            case TRAINER_ENCOUNTER_MUSIC_RICH:
                ...
                break;
            case TRAINER_ENCOUNTER_MUSIC_FAIRY:
                music = MUS_ZGS_ENCOUNTER_FAIRY;
                break;
            case TRAINER_ENCOUNTER_MUSIC_TENGU:
                music = MUS_ZGS_ENCOUNTER_TENGU;
                break;
            default:
                music = MUS_ENCOUNTER_SUSPICIOUS;
            }
...
u8 NuzlockeIsCaptureBlockedBySpeciesClause(u16 species) // @Kurausukun
{
    ...
    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT))
        return TRUE; //player already has this exact pokemon
...
void SetNuzlockeChecks(void)
{
    if (IsNuzlockeActive())
    {
        ...
        #ifndef NDEBUG
        MgbaPrintf(MGBA_LOG_DEBUG, "gNuzlockeIsCaptureBlocked=%d", gNuzlockeIsCaptureBlocked);
        MgbaPrintf(MGBA_LOG_DEBUG, "gNuzlockeIsSpeciesClauseActive=%d", gNuzlockeIsSpeciesClauseActive);
        #endif
    }
    else
    {
