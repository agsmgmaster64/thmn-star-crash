...
const struct TerrainInfo gBattleTerrainInfo[B_TERRAIN_COUNT] = {
    [B_TERRAIN_GRASSY] = {
        ...
        .naturePowerMove = MOVE_ENERGY_LIGHT,
    ...
    [B_TERRAIN_MISTY] = {
        ...
        .naturePowerMove = MOVE_SIGNAL_BEAM,
    ...
    [B_TERRAIN_PSYCHIC] = {
        .type = TYPE_REASON,
        .secretPowerAnimation = gBattleAnimMove_Psyshot,
        .secretPowerEffect = SECRET_POWER_SPD_MINUS_1,
        .naturePowerMove = MOVE_MANA_BURST,
    ...
    [B_TERRAIN_HOLY] = {
        .type = TYPE_FAITH,
        .secretPowerAnimation = gBattleAnimMove_Extrasensory,
        .secretPowerEffect = SECRET_POWER_SPD_MINUS_1,
        .naturePowerMove = MOVE_EXTRASENSORY,
        .battleBackground = BG_HOLY_TERRAIN,
        .seedStat = STAT_SPDEF,
        .seedHoldEffect = HOLD_EFFECT_PARAM_PSYCHIC_TERRAIN,
        .startMessage = B_MSG_TERRAIN_SET_HOLY,
        .endMessage = B_MSG_TERRAIN_END_HOLY,
    },
};
...
u32 TrySetCantSelectMoveBattleScript(enum BattlerId battler)
{
    ...
    if (dynamaxBypassCheck && (GetBattlerAbility(battler) == ABILITY_ONI_TACTICS) && *choicedMove != MOVE_NONE
...
bool32 TryFieldEffects(enum FieldEffectCases caseId)
{
    ...
    case FIELD_EFFECT_TRAINER_STATUSES:  // starting field/side/etc statuses with a variable
        ...
        {
            effect = SetStartingFieldTerrain(B_TERRAIN_HOLY, &gFieldTimers.terrainTimer,
                gStartingStatuses.holyTerrain ? 0 : 5);
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_HOLY_SURGE:
            if (!shouldAbilityTrigger)
                break;
            if (TryChangeBattleTerrain(battler, B_TERRAIN_HOLY))
            {
    ...
    case ABILITYEFFECT_COLOR_CHANGE:
        ...
        case ABILITY_DETERMINATOR:
            ...
            {
                gEffectBattler = gBattlerAbility = battler;
                if (CompareStat(battler, STAT_DEF, MAX_STAT_STAGE, CMP_LESS_THAN, ABILITY_NONE))
                    SetStatChange(battler, STAT_DEF, 1);

                if (CompareStat(battler, STAT_SPDEF, MAX_STAT_STAGE, CMP_LESS_THAN, ABILITY_NONE))
                    SetStatChange(battler, STAT_SPDEF, 1);
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker. battler should be the target instead of the attacker.
        ...
        */
        case ABILITY_LEAF_SPICE:
            if (IsBattlerAlive(battler)
             && !gBattleStruct->unableToUseMove
             && CanBeBurned(gBattlerAttacker, battler, GetBattlerAbility(battler))
        ...
        case ABILITY_DOUR_ECHO:
             ...
             && IsSoundMove(gCurrentMove))
            {
                ...
                if (gBattleMons[battler].attack < gBattleMons[battler].spAttack)
                {
                    ...
                    if (TryStatChange(&cv, &st) == STAT_CHANGE_WORKED || cv.abilities[battler] == ABILITY_MIRROR_ARMOR)
                    {
        ...
        case ABILITY_MAGICIAN:
             ...
             && !gSpecialStatuses[battler].gemBoost) // In base game, gems are consumed after magician would activate.
            {
                ...
                for (u32 i = 0; i < gBattlersCount; i++)
                {
                    ...
                    if (GetBattlerAbility(targetBattler) == ABILITY_COLLECTOR)
                    {
                        effect = FALSE;
                        break;
                    }
...
{
    return GetBattlerAbility(battler) == ABILITY_FRETFUL
...
void UpdateTruantToggleForAbilityChange(enum BattlerId battler, enum Ability oldAbility)
{
    ...
    if (oldAbility == ABILITY_FRETFUL && newAbility != ABILITY_FRETFUL)
    {
        if (GetConfig(B_TRUANT) >= GEN_4)
            gBattleMons[battler].volatiles.truantToggle = 0;
    }
    else if (oldAbility != ABILITY_FRETFUL && newAbility == ABILITY_FRETFUL)
    {
...
{
    if (GetConfig(B_TRUANT) >= GEN_4 && gBattleMons[battler].ability == ABILITY_FRETFUL)
        gBattleMons[battler].volatiles.truantToggle = 0;
}
...
void UpdateTruantTogglesOnNeutralizingGasEnd(void)
{
    ...
    {
        if (IsBattlerAlive(battler)
         && gBattleMons[battler].ability == ABILITY_FRETFUL
...
bool32 IsHolyTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, enum BattleTerrain currTerrain)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, B_TERRAIN_HOLY, currTerrain);
}
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    if (GetMoveEffect(ctx->chosenMove) == EFFECT_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (IsGrassyTerrainAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->terrain) && moveType == TYPE_NATURE)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsMistyTerrainAffected(battlerDef, ctx->abilities[battlerAtk], ctx->holdEffects[battlerDef], ctx->terrain) && moveType == TYPE_HEART)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsElectricTerrainAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->terrain) && moveType == TYPE_ELECTRIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsPsychicTerrainAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->terrain) && moveType == TYPE_REASON)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsHolyTerrainAffected(battlerAtk, ctx->abilities[battlerAtk], ctx->holdEffects[battlerAtk], ctx->terrain) && moveType == TYPE_FAITH)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    ...
    if ((moveType == TYPE_DARK && DmgCalc_IsAbilityOnField(ABILITY_DARK_AURA, ctx->abilities, &fieldAbilityBattler))
     || (moveType == TYPE_HEART && DmgCalc_IsAbilityOnField(ABILITY_FAIRY_AURA, ctx->abilities, &fieldAbilityBattler)))
    {
        if (ctx->updateFlags)
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    if (GetConfig(B_SANDSTORM_SPDEF_BOOST) >= GEN_4
	 ...
	 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_BEAST)
	 && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline bool32 IsBattlerBigCloudAffected(enum BattlerId battler, enum HoldEffect holdEffect)
{
...
static inline bool32 IsBattlerBloomersAffected(enum BattlerId battler, enum HoldEffect holdEffect)
{
...
s32 CalcCritChanceStage(struct DamageContext *ctx)
{
    ...
    {
        if (ctx->abilities[ctx->battlerDef] == ABILITY_GUARD_ARMOR || ctx->abilities[ctx->battlerDef] == ABILITY_SHELL_ARMOR)
        {
        ...
        if (ctx->holdEffects[ctx->battlerDef] == HOLD_EFFECT_LUCKY_SHIELD)
        {
            if (ctx->updateFlags)
            {
                if (critChance == CRITICAL_HIT_ALWAYS)
...
s32 CalcCritChanceStageGen1(struct DamageContext *ctx)
{
    ...
    else if (ctx->holdEffects[ctx->battlerDef] == HOLD_EFFECT_LUCKY_SHIELD)
    {
        if (ctx->updateFlags)
            RecordItemEffectBattle(ctx->battlerDef, ctx->holdEffects[ctx->battlerDef]);
        critChance = CRITICAL_HIT_BLOCKED;
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    if (GetMoveEffect(ctx->move) == EFFECT_SUPER_EFFECTIVE_ON_ARG && defType == GetMoveArgType(ctx->move) && !ctx->isAnticipation)
        mod = UQ_4_12(2.0);
    if (ctx->moveType == TYPE_NATURE && (defType == TYPE_NATURE || defType == TYPE_FLYING) && ctx->abilities[ctx->battlerAtk] == ABILITY_NATURE_FROST)
    {
        mod = UQ_4_12(2.0);
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerAtk, ctx->abilities[ctx->battlerAtk]);
    }
    if (ctx->moveType == TYPE_EARTH && defType == TYPE_FLYING && IsBattlerGrounded(ctx->battlerDef, ctx->abilities[ctx->battlerDef], ctx->holdEffects[ctx->battlerDef]) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_EARTH
    ...
    if (((ctx->abilities[ctx->battlerDef] == ABILITY_PLAY_GHOST && modifier <= UQ_4_12(1.0) && !isPresentHealing)
...
uq4_12_t GetOverworldTypeEffectiveness(struct Pokemon *mon, enum Type moveType)
{
    ...
    if ((modifier <= UQ_4_12(1.0) && ctx.abilities[B_BATTLER_0] == ABILITY_PLAY_GHOST)
