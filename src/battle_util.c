    ...
    else
    {
        ...
        if (gBattleWeather & B_WEATHER_PRIMAL_ANY)
            gBattleStruct->weatherDuration = 0;
               ...
               || GetBattlerHoldEffect(battler) == HOLD_EFFECT_SCARLET_ROCK)
...
bool32 ChangeTypeBasedOnTerrain(enum BattlerId battler)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        ...
        battlerType = TYPE_NATURE;
    else if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
        battlerType = TYPE_HEART;
    else if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
        battlerType = TYPE_REASON;
    else if (gFieldStatuses & STATUS_FIELD_HOLY_TERRAIN)
        battlerType = TYPE_FAITH;
    else // failsafe
        return FALSE;
...
bool32 TryFieldEffects(enum FieldEffectCases caseId)
{
    ...
    case FIELD_EFFECT_TRAINER_STATUSES:  // starting field/side/etc statuses with a variable
        ...
        else if (gStartingStatuses.holyTerrain || gStartingStatuses.holyTerrainTemporary)
        {
            effect = SetStartingFieldStatus(
                        STATUS_FIELD_HOLY_TERRAIN,
                        B_MSG_TERRAIN_SET_HOLY,
                        0,
                        &gFieldTimers.terrainTimer, gStartingStatuses.holyTerrain ? 0 : 5);
            gStartingStatuses.holyTerrainTemporary = gStartingStatuses.holyTerrain = FALSE;
            isTerrain = TRUE;
        }
        ...
        else if (gStartingStatuses.inverseRoom || gStartingStatuses.inverseRoomTemporary)
        {
            effect = SetStartingFieldStatus(
                        STATUS_FIELD_INVERSE_ROOM,
                        B_MSG_SET_INVERSE_ROOM,
                        B_ANIM_INVERSE_ROOM,
                        &gFieldTimers.inverseRoomTimer,  gStartingStatuses.inverseRoom ? 0 : 5);
            gStartingStatuses.inverseRoomTemporary = gStartingStatuses.inverseRoom = FALSE;
        }
...
static bool32 TryConcerto(void)
{
    bool32 anyConcertoQueued = FALSE;
    enum BattlerId concertoBattler = MAX_BATTLERS_COUNT;
    ...
    for (enum BattlerId battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        {
            if (!anyConcertoQueued || (gBattleMons[battler].speed < gBattleMons[concertoBattler].speed))
                concertoBattler = battler;
    ...
    if (IsBattlerAlive(concertoBattler)
     && !gSpecialStatuses[concertoBattler].concertoUsedMove
     && gBattlerAttacker != concertoBattler)
    {
        gSpecialStatuses[concertoBattler].concertoUsedMove = TRUE;
        gSpecialStatuses[concertoBattler].backUpTarget = gBattleStruct->moveTarget[concertoBattler] + 1;
        gBattleMons[concertoBattler].volatiles.activateDancer = FALSE;
        gBattlerAttacker = gBattlerAbility = concertoBattler;
        gCalledMove = gCurrentMove;
        gLastUsedAbility = ABILITY_DANCER;
        RecordAbilityBattle(gBattlerAttacker, ABILITY_DANCER);
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_COMATOSE:
            ...
            break;
        case ABILITY_LUCID_DREAMING:
            if (shouldAbilityTrigger)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_LUCIDDREAMING;
    ...
    case ABILITYEFFECT_COLOR_CHANGE:
        ...
        case ABILITY_ANGER_SHELL:
            ...
            break;
        case ABILITY_DETERMINATOR:
            if (IsBattlerTurnDamaged(battler, EXCLUDING_SUBSTITUTES)
             && IsBattlerAlive(battler)
             && HadMoreThanHalfHpNowDoesnt(battler))
            {
                gEffectBattler = gBattlerAbility = battler;
                BattleScriptCall(BattleScript_DeterminatorActivates);
                effect++;
            }
            break;
        default:
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_RATTLED:
            if (IsBattlerTurnDamaged(battler, EXCLUDING_SUBSTITUTES)
             && IsBattlerAlive(battler)
             && (moveType == TYPE_DARK || moveType == TYPE_BEAST || moveType == TYPE_GHOST)
        ...
        case ABILITY_PERISH_BODY:
            ...
            break;
        case ABILITY_FOUL_AROMA:
            if (IsBattlerAlive(gBattlerAttacker)
             && !gBattleStruct->unableToUseMove
             && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerHoldEffect(gBattlerAttacker), move)
             && IsBattlerTurnDamaged(gBattlerTarget, EXCLUDING_SUBSTITUTES)
             && CanBeConfused(gBattlerAttacker, gBattlerAttacker)
             && (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_FOUL_AROMA, 30) : RandomChance(RNG_FOUL_AROMA, 1, 3)))
            {
                gEffectBattler = gBattlerAttacker;
                gBattleScripting.battler = gBattlerTarget;
             ...
             && !gSpecialStatuses[battler].gemBoost) // In base game, gems are consumed after magician would activate.
            {
                ...
                for (enum BattlerId battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
                {
                    if (gBattleMons[battlerDef].item != ITEM_NONE
                     ...
                     && (GetBattlerAbility(battlerDef) != ABILITY_COLLECTOR || !IsBattlerAlive(battlerDef)))
                    {
        ...
        case ABILITY_BEAST_BOOST:
            {
                ...
                else if (ability == ABILITY_AMBITION)
                    stat = STAT_SPATK;
...
bool32 IsHolyTerrainAffected(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect, u32 fieldStatuses)
{
    return IsBattlerTerrainAffected(battler, ability, holdEffect, fieldStatuses, STATUS_FIELD_HOLY_TERRAIN);
}
...
bool32 CanSetNonVolatileStatus(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum ResultOption option)
{
    ...
    case MOVE_EFFECT_TOXIC:
        ...
        else if (abilityAtk != ABILITY_CORROSION && IS_BATTLER_ANY_TYPE(battlerDef, TYPE_MIASMA, TYPE_STEEL))
        {
    ...
    case MOVE_EFFECT_FREEZE:
        ...
        else if (abilityDef == ABILITY_FIRE_VEIL)
        {
            abilityAffected = TRUE;
            battleScript = BattleScript_NotAffected;
        }
    case MOVE_EFFECT_FROSTBITE:
        ...
        {
            battleScript = BattleScript_AlreadyFrostbit;
        }
...
const struct TypePower gNaturalGiftTable[] =
{
    ...
    [ITEM_TO_BERRY(ITEM_LEPPA_BERRY)] = {TYPE_DREAM, 80},
    [ITEM_TO_BERRY(ITEM_ORAN_BERRY)] = {TYPE_MIASMA, 80},
    [ITEM_TO_BERRY(ITEM_PERSIM_BERRY)] = {TYPE_EARTH, 80},
    [ITEM_TO_BERRY(ITEM_LUM_BERRY)] = {TYPE_FLYING, 80},
    [ITEM_TO_BERRY(ITEM_SITRUS_BERRY)] = {TYPE_REASON, 80},
    [ITEM_TO_BERRY(ITEM_FIGY_BERRY)] = {TYPE_HEART, 80},
    ...
    [ITEM_TO_BERRY(ITEM_SALAC_BERRY)] = {TYPE_DREAM, 100},
    [ITEM_TO_BERRY(ITEM_PETAYA_BERRY)] = {TYPE_MIASMA, 100},
    [ITEM_TO_BERRY(ITEM_APICOT_BERRY)] = {TYPE_EARTH, 100},
    [ITEM_TO_BERRY(ITEM_LANSAT_BERRY)] = {TYPE_FLYING, 100},
    [ITEM_TO_BERRY(ITEM_STARF_BERRY)] = {TYPE_REASON, 100},
    [ITEM_TO_BERRY(ITEM_ENIGMA_BERRY)] = {TYPE_HEART, 100},
    [ITEM_TO_BERRY(ITEM_MICLE_BERRY)] = {TYPE_BEAST, 100},
    [ITEM_TO_BERRY(ITEM_CUSTAP_BERRY)] = {TYPE_GHOST, 100},
    [ITEM_TO_BERRY(ITEM_JABOCA_BERRY)] = {TYPE_FAITH, 100},
    [ITEM_TO_BERRY(ITEM_ROWAP_BERRY)] = {TYPE_DARK, 100},
    [ITEM_TO_BERRY(ITEM_KEE_BERRY)] = {TYPE_COSMIC, 100},
    [ITEM_TO_BERRY(ITEM_MARANGA_BERRY)] = {TYPE_DARK, 100},
    [ITEM_TO_BERRY(ITEM_PUMKIN_BERRY)] = {TYPE_REASON, 100},
    [ITEM_TO_BERRY(ITEM_DRASH_BERRY)] = {TYPE_HEART, 100},
    [ITEM_TO_BERRY(ITEM_EGGANT_BERRY)] = {TYPE_WATER, 100},
    [ITEM_TO_BERRY(ITEM_STRIB_BERRY)] = {TYPE_NATURE, 100},
    [ITEM_TO_BERRY(ITEM_TURLAN_BERRY)] = {TYPE_GHOST, 100},
    [ITEM_TO_BERRY(ITEM_NUTPEA_BERRY)] = {TYPE_EARTH, 100},
    [ITEM_TO_BERRY(ITEM_IONNO_BERRY)] = {TYPE_MIASMA, 100},
    [ITEM_TO_BERRY(ITEM_KUO_BERRY)] = {TYPE_DARK, 100},
    [ITEM_TO_BERRY(ITEM_YAGO_BERRY)] = {TYPE_COSMIC, 100},
    [ITEM_TO_BERRY(ITEM_TOUGA_BERRY)] = {TYPE_FIRE, 100},
    [ITEM_TO_BERRY(ITEM_GARLIG_BERRY)] = {TYPE_MIASMA, 100},
    [ITEM_TO_BERRY(ITEM_TOPO_BERRY)] = {TYPE_DARK, 100},
};
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    if (GetMoveEffect(ctx->chosenMove) == EFFECT_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (IsGrassyTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_NATURE)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsMistyTerrainAffected(battlerDef, ctx->abilityDef, ctx->holdEffectDef, ctx->fieldStatuses) && moveType == TYPE_HEART)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsElectricTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_ELECTRIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsPsychicTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_REASON)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsHolyTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_FAITH)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    ...
    case ABILITY_PIXILATE:
        if (moveType == TYPE_HEART && gBattleStruct->battlerState[battlerAtk].ateBoost)
    ...
    case ABILITY_NORMALIZE:
        if (moveType == TYPE_ILLUSION && gBattleStruct->battlerState[battlerAtk].ateBoost && GetConfig(B_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    ...
    if ((IsAbilityOnField(ABILITY_DARK_AURA) && moveType == TYPE_DARK)
     || (IsAbilityOnField(ABILITY_FAIRY_AURA) && moveType == TYPE_HEART))
    {
    ...
    case HOLD_EFFECT_DOUBLE_SOUL:
        if ((gBattleMons[battlerAtk].species == SPECIES_MALE_SHINGYOKU || gBattleMons[battlerAtk].species == SPECIES_FEMALE_SHINGYOKU)
...
static inline u32 CalcAttackStat(struct BattleContext *ctx)
{
    ...
    case HOLD_EFFECT_DARK_RIBBON:
        if (IsBattleMovePhysical(move) && (atkBaseSpeciesId == SPECIES_CHIBI_HINA  || atkBaseSpeciesId == SPECIES_NORMAL_HINA
         || atkBaseSpeciesId == SPECIES_ATTACK_HINA || atkBaseSpeciesId == SPECIES_DEFENSE_HINA))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_KUSANAGI:
        if (atkBaseSpeciesId == SPECIES_RINNOSUKE && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_ICY_BALL:
        if (atkBaseSpeciesId == SPECIES_CHIBI_CIRNO && (GetConfig(B_LIGHT_BALL_ATTACK_BOOST) >= GEN_4 || IsBattleMoveSpecial(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline u32 CalcDefenseStat(struct BattleContext *ctx)
{
    ...
    if (GetConfig(B_SANDSTORM_SPDEF_BOOST) >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_BEAST) && IsBattlerWeatherAffected(ctx->holdEffectDef, ctx->weather, B_WEATHER_SANDSTORM) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline uq4_12_t GetDefenderItemsModifier(struct BattleContext *ctx)
{
    ...
    case HOLD_EFFECT_RESIST_BERRY:
        if (IsUnnerveBlocked(ctx->battlerDef, gBattleMons[ctx->battlerDef].item))
            return UQ_4_12(1.0);
        if (ctx->moveType == GetBattlerHoldEffectParam(ctx->battlerDef) && (ctx->moveType == TYPE_ILLUSION || ctx->typeEffectivenessModifier >= UQ_4_12(2.0)))
        {
            if (ctx->updateFlags)
...
s32 DoFixedDamageMoveCalc(struct BattleContext *ctx)
{
    ...
    gBattleStruct->moveResultFlags[ctx->battlerDef] &= ~(MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE);
...
static inline u32 IsBattlerBigCloudAffected(enum BattlerId battler, enum HoldEffect holdEffect)
{
...
static inline u32 IsBattlerBloomersAffected(enum BattlerId battler, enum HoldEffect holdEffect)
{
...
s32 CalcCritChanceStage(struct BattleContext *ctx)
{
    ...
    {
        if (ctx->abilityDef == ABILITY_GUARD_ARMOR || ctx->abilityDef == ABILITY_SHELL_ARMOR)
        {
        ...
        if (ctx->holdEffectDef == HOLD_EFFECT_LUCKY_SHIELD)
        {
            if (ctx->updateFlags)
            {
                if (critChance == CRITICAL_HIT_ALWAYS)
...
s32 CalcCritChanceStageGen1(struct BattleContext *ctx)
{
    ...
    else if (ctx->holdEffectDef == HOLD_EFFECT_LUCKY_SHIELD)
    {
        if (ctx->updateFlags)
            RecordItemEffectBattle(ctx->battlerDef, ctx->holdEffectDef);
        critChance = CRITICAL_HIT_BLOCKED;
...
static inline void MulByTypeEffectiveness(struct BattleContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    else if ((ctx->moveType == TYPE_DARK || ctx->moveType == TYPE_ILLUSION) && defType == TYPE_GHOST && gBattleMons[ctx->battlerDef].volatiles.foresight && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
    }
    else if ((ctx->moveType == TYPE_DARK || ctx->moveType == TYPE_ILLUSION) && defType == TYPE_GHOST
    ...
    if (ctx->moveType == TYPE_REASON && defType == TYPE_DARK && gBattleMons[ctx->battlerDef].volatiles.miracleEye && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (GetMoveEffect(ctx->move) == EFFECT_SUPER_EFFECTIVE_ON_ARG && defType == GetMoveArgType(ctx->move) && !ctx->isAnticipation)
        mod = UQ_4_12(2.0);
    if (ctx->moveType == TYPE_NATURE && (defType == TYPE_NATURE || defType == TYPE_FLYING) && ctx->abilityAtk == ABILITY_NATURE_FROST)
    {
        mod = UQ_4_12(2.0);
        if (ctx->updateFlags)
            RecordAbilityBattle(ctx->battlerAtk, ctx->abilityAtk);
    }
    if (ctx->moveType == TYPE_EARTH && defType == TYPE_FLYING && IsBattlerGrounded(ctx->battlerDef, ctx->abilityDef, ctx->holdEffectDef) && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
...
void UpdateMoveResultFlags(uq4_12_t modifier, u16 *resultFlags)
{
    ...
    {
        *resultFlags &= ~(MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE | MOVE_RESULT_DOESNT_AFFECT_FOE);
    }
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct BattleContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_EARTH && !IsBattlerGroundedInverseCheck(ctx->battlerDef, ctx->abilityDef, ctx->holdEffectDef, INVERSE_BATTLE, ctx->isAnticipation) && !(MoveIgnoresTypeIfFlyingAndUngrounded(ctx->move)))
    {
        modifier = UQ_4_12(0.0);
    ...
    if (((ctx->abilityDef == ABILITY_PLAY_GHOST && modifier <= UQ_4_12(1.0) && !isPresentHealing)
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(enum Move move, enum Species speciesDef, enum Ability abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_NONE)
    {
        ...
        if (ctx.moveType == TYPE_EARTH && abilityDef == ABILITY_LEVITATE && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
...
uq4_12_t GetOverworldTypeEffectiveness(struct Pokemon *mon, enum Type moveType)
{
    ...
    if ((modifier <= UQ_4_12(1.0) && ctx.abilityDef == ABILITY_PLAY_GHOST)
     || CanAbilityAbsorbMove(&ctx))
        modifier = UQ_4_12(0.0);
...
bool32 IsBattlerAffectedByHazards(enum BattlerId battler, enum HoldEffect holdEffect, bool32 toxicSpikes)
{
    ...
    else if (toxicSpikes && holdEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS && !IS_BATTLER_OF_TYPE(battler, TYPE_MIASMA))
    {
        ret = FALSE;
...
bool32 IsAffectedByPowderMove(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect)
{
    if (GetConfig(B_POWDER_OVERCOAT) >= GEN_6 && ability == ABILITY_OVERCOAT)
        return FALSE;
    if (GetConfig(B_POWDER_GRASS) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NATURE))
        return FALSE;
...
bool32 IsNaturalEnemy(enum Species speciesAttacker, enum Species speciesTarget)
{
    ...
    {
    case SPECIES_CHIBI_MOKOU:
    case SPECIES_NORMAL_MOKOU:
    case SPECIES_ATTACK_MOKOU:
    case SPECIES_DEFENSE_MOKOU:
    case SPECIES_ADVENT_MOKOU:
        return (speciesTarget == SPECIES_CHIBI_KAGUYA)
            || (speciesTarget == SPECIES_NORMAL_KAGUYA)
            || (speciesTarget == SPECIES_ATTACK_KAGUYA)
            || (speciesTarget == SPECIES_DEFENSE_KAGUYA);
    case SPECIES_CHIBI_KAGUYA:
    case SPECIES_NORMAL_KAGUYA:
    case SPECIES_ATTACK_KAGUYA:
    case SPECIES_DEFENSE_KAGUYA:
        return (speciesTarget == SPECIES_CHIBI_MOKOU)
            || (speciesTarget == SPECIES_NORMAL_MOKOU)
            || (speciesTarget == SPECIES_ATTACK_MOKOU)
            || (speciesTarget == SPECIES_DEFENSE_MOKOU)
            || (speciesTarget == SPECIES_ADVENT_MOKOU);
    default:
        return FALSE;
    }
    return FALSE;
