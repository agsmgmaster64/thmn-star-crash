    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_FASCINATE:
            if (shouldAbilityTrigger && !IsOpposingSideEmpty(battler))
            {
                ...
                BattleScriptPushCursorAndCallback(BattleScript_FascinateActivates);
                effect++;
            }
            break;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    case ABILITY_NORMALIZE:
        if (moveType == TYPE_ILLUSION && gBattleStruct->battlerState[battlerAtk].ateBoost && GetConfig(CONFIG_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
...
static inline u32 CalcAttackStat(struct BattleContext *ctx)
{
    ...
    case HOLD_EFFECT_ICY_BALL:
        if (atkBaseSpeciesId == SPECIES_CHIBI_CIRNO && (GetConfig(CONFIG_LIGHT_BALL_ATTACK_BOOST) >= GEN_4 || IsBattleMoveSpecial(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline u32 CalcDefenseStat(struct BattleContext *ctx)
{
    ...
    if (GetConfig(CONFIG_SANDSTORM_SPDEF_BOOST) >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_BEAST) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    // hail def boost for ice types
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE) && IsBattlerWeatherAffected(battlerDef, (B_WEATHER_HAIL | B_WEATHER_SNOW)) && usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct BattleContext *ctx, uq4_12_t modifier)
{
    ...
    if (((ctx->abilityDef == ABILITY_PLAY_GHOST && modifier <= UQ_4_12(1.0))
...
bool32 IsAffectedByPowderMove(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect)
{
    if (GetConfig(CONFIG_POWDER_OVERCOAT) >= GEN_6 && ability == ABILITY_OVERCOAT)
        return FALSE;
    if (GetConfig(CONFIG_POWDER_GRASS) >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NATURE))
        return FALSE;
