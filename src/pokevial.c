...
void PokevialSizeSet(u8 sizeSet)
{
    if (sizeSet > VIAL_MAX_SIZE)
        gSaveBlock1Ptr->pokevial.Size = VIAL_MAX_SIZE;
    else
        gSaveBlock1Ptr->pokevial.Size = sizeSet;
    PokevialFixDoseOverflow();
}

void PokevialDoseSet(u8 doseSet)
{
    if (doseSet > gSaveBlock1Ptr->pokevial.Size)
        gSaveBlock1Ptr->pokevial.Dose = gSaveBlock1Ptr->pokevial.Size;
    else
        gSaveBlock1Ptr->pokevial.Dose = doseSet;
}
