...
static const u8 sNewShopMenu_SellerMugshotGfx_Nitori[] = INCGFX_U8("graphics/new_shop/sellers/nitori/mugshot.png", ".4bpp");
static const u16 sNewShopMenu_SellerMugshotPal_Nitori[] = INCGFX_U16("graphics/new_shop/sellers/nitori/mugshot.png", ".gbapal");
static const u32 sNewShopMenu_SellerMenuGfx_Nitori[] = INCGFX_U32("graphics/new_shop/sellers/nitori/menu.png", ".4bpp.smol");
static const u32 sNewShopMenu_SellerMenuCoinGfx_Nitori[] = INCGFX_U32("graphics/new_shop/sellers/nitori/menu_coin.png", ".4bpp.smol");
static const u16 sNewShopMenu_SellerMenuPal_Nitori[] = INCGFX_U16("graphics/new_shop/sellers/nitori/menu.png", ".gbapal");
static const u32 sNewShopMenu_SellerMenuMap_Nitori[] = INCBIN_U32("graphics/new_shop/sellers/nitori/menu.bin.smolTM");
static const u32 sNewShopMenu_SellerScrollGfx_Nitori[] = INCGFX_U32("graphics/new_shop/sellers/nitori/scroll.png", ".4bpp.smol");
static const u16 sNewShopMenu_SellerScrollPal_Nitori[] = INCGFX_U16("graphics/new_shop/sellers/nitori/scroll.png", ".gbapal");
static const u32 sNewShopMenu_SellerScrollMap_Nitori[] = INCBIN_U32("graphics/new_shop/sellers/nitori/scroll.bin.smolTM");
static const u16 sNewShopMenu_SellerCursorGfx_Nitori[] = INCGFX_U16("graphics/new_shop/sellers/nitori/cursor.png", ".4bpp");
static const u16 sNewShopMenu_SellerCursorPal_Nitori[] = INCGFX_U16("graphics/new_shop/sellers/nitori/cursor.png", ".gbapal");
...
static const struct Seller sSellers[] = {
    ...
    [SELLER_NITORI] =
    {
        ...
        .menuPointGfx = sNewShopMenu_SellerMenuCoinGfx_Nitori,
...
{
    return martType == NEW_SHOP_TYPE_MOVE
        || martType == NEW_SHOP_TYPE_MOVE_VARIABLE
        || martType == NEW_SHOP_TYPE_MOVE_COINS
        || martType == NEW_SHOP_TYPE_MOVE_POINTS;
}
...
static inline bool32 CanMartTypeReturnToShopMenu(void)
{
    return !sMartInfo.buyOnly;
}
...
{
    if (sMartInfo.martType == NEW_SHOP_TYPE_SELL_ONLY)
    {
...
void CB2_InitBuyMenuAfterTutor(void)
{
    if (gSpecialVar_Result == TRUE)
    {
        ...
        {
        case NEW_SHOP_TYPE_MOVE_POINTS:
            RemoveBattlePoints(gSpecialVar_0x8008);
            break;
        case NEW_SHOP_TYPE_MOVE_COINS:
            RemoveCoins(gSpecialVar_0x8008);
            break;
        default:
...
static inline u32 BuyMenuGetItemPrice(u32 id)
{
    switch (sMartInfo.martType)
    {
        ...
        // custom
        case NEW_SHOP_TYPE_VARIABLE:
        case NEW_SHOP_TYPE_MOVE_VARIABLE:
...
static void BuyMenuInitWindows(void)
{
    ...
    if (!IsMartTypeMove(sMartInfo.martType))
        BuyMenuPrint(WIN_MULTI, COMPOUND_STRING("PRICE"), 0, ITEM_PRICE_Y, TEXT_SKIP_DRAW, COLORID_NORMAL, FALSE);
    if (IsMartTypeItem(sMartInfo.martType))
        BuyMenuPrint(WIN_MULTI, COMPOUND_STRING("IN BAG"), 0, ITEM_IN_BAG_Y, TEXT_SKIP_DRAW, COLORID_NORMAL, FALSE);
...
static void UpdateItemData(void)
{
    ...
    if (sMartInfo.itemList[GridMenu_SelectedIndex(sShopData->gridItems)] == ITEM_NONE)
    {
        ...
        BuyMenuPrint(WIN_ITEM_DESCRIPTION, gText_QuitShopping, 8, 0, TEXT_SKIP_DRAW, COLORID_NORMAL, TRUE);
    }
    else
    {
        ...
        BuyMenuPrint(WIN_MULTI, BuyMenuGetItemName(i), 0, ITEM_NAME_Y, TEXT_SKIP_DRAW, COLORID_NORMAL, FALSE);
...
static void BuyMenuDisplayMessage(u8 taskId, const u8 *str, TaskFunc nextFunc)
{
    ...
    BuyMenuPrint(WIN_ITEM_DESCRIPTION, gStringVar4, 8, 0, TEXT_SKIP_DRAW, COLORID_NORMAL, TRUE);
...
{
    if (CanMartTypeReturnToShopMenu())
        gFieldCallback = MapPostLoadHook_ReturnToShopMenu;
...
static void Task_ReturnToItemListAfterItemPurchase(u8 taskId)
{
    ...
    if (IsMartTypeItem(sMartInfo.martType) && GetItemPocket(sShopData->currentItemId) == POCKET_POKE_BALLS)
    {
        if (IsTextPrinterActiveOnWindow(WIN_ITEM_DESCRIPTION))
        {
            return;
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    BuyMenuPrint(WIN_QUANTITY_PRICE, gStringVar4, x, 2, TEXT_SKIP_DRAW, COLORID_NORMAL, FALSE);

    PrintMoneyLocal(WIN_QUANTITY_PRICE, RIGHT_ALIGNED_X, 14, sShopData->totalCost, COLORID_NORMAL, STR_CONV_MODE_LEFT_ALIGN, FALSE);
...
{
    sMartInfo.martType = NEW_SHOP_TYPE_NORMAL;
...
{
    sMartInfo.martType = NEW_SHOP_TYPE_VARIABLE;
    sMartInfo.buyOnly = TRUE;
    SetShopItemsForSale(itemsForSale);
    ClearItemPurchases();
    CreateTask(Task_HandleShopMenuBuy, 8);  //skip mart multichoice. go right to shop buy window
...
{
    sMartInfo.martType = NEW_SHOP_TYPE_SELL_ONLY;
    CreateTask(Task_HandleShopMenuSell, 8);  //skip mart multichoice. go right to shop sell window
}

void NewShop_CreateMoveTutorPointsMenu(const u16 *itemsForSale)
{
    CreateShopMenu(NEW_SHOP_TYPE_MOVE_POINTS);
    SetShopItemsForSale(itemsForSale);
    SetShopMenuCallback(ScriptContext_Enable);
}

void NewShop_CreateMoveTutorVariableMenu(const u16 *itemsForSale)
{
    CreateShopMenu(NEW_SHOP_TYPE_MOVE_VARIABLE);
    SetShopItemsForSale(itemsForSale);
    SetShopMenuCallback(ScriptContext_Enable);
}

void NewShop_CreateMoveTutorCoinsMenu(const u16 *itemsForSale)
{
    CreateShopMenu(NEW_SHOP_TYPE_MOVE_COINS);
    SetShopItemsForSale(itemsForSale);
    SetShopMenuCallback(ScriptContext_Enable);
}
...
{
    CreateShopMenu(NEW_SHOP_TYPE_MOVE);
    SetShopItemsForSale(itemsForSale);
    SetShopMenuCallback(ScriptContext_Enable);
}
