...
static const u8 sFontColorTable[][3] = {
    ...
    [COLORID_DESCRIPTION] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY,  TEXT_COLOR_WHITE},
...
static void PrintItemDescription(int itemIndex)
{
    ...
    BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, str, 3, 0, 0, 0, 0, COLORID_DESCRIPTION);
}
    ...
    default:
        ...
        else
        {
            ...
            case POCKET_KEY_ITEMS:
                ...
                else if (GetItemFieldFunc(gSpecialVar_ItemId) == ItemUseOutOfBattle_Bike)
                {
                    gBagMenu->contextMenuNumItems = ARRAY_COUNT(sContextMenuItems_Bike);
...
static void ItemMenu_RegisterL(u8 taskId)
{
    if (gSaveBlock3Ptr->registeredItemL == gSpecialVar_ItemId)
        gSaveBlock3Ptr->registeredItemL = ITEM_NONE;
    else
        gSaveBlock3Ptr->registeredItemL = gSpecialVar_ItemId;
...
static void ItemMenu_RegisterR(u8 taskId)
{
    if (gSaveBlock3Ptr->registeredItemR == gSpecialVar_ItemId)
        gSaveBlock3Ptr->registeredItemR = ITEM_NONE;
    else
        gSaveBlock3Ptr->registeredItemR = gSpecialVar_ItemId;
...
static void TryDepositItem(u8 taskId)
{
    ...
    if (GetItemImportance(gSpecialVar_ItemId))
    {
        ...
        BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, sText_CantStoreImportantItems, 3, 0, 0, 0, 0, COLORID_DESCRIPTION);
    ...
    {
        // No room to deposit
        BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, sText_NoRoomForItems, 3, 0, 0, 0, 0, COLORID_DESCRIPTION);
        gTasks[taskId].func = WaitDepositErrorMessage;
    }
}
...
static void Task_WallyTutorialBagMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        case WALLY_BAG_DELAY * 2:
            PlaySE(SE_SELECT);
            BagMenu_PrintCursor(tListTaskId, COLORID_GRAY_CURSOR);
            gSpecialVar_ItemId = ITEM_TOHO_ORB;
            OpenContextMenu(taskId);
            tTimer++;
            break;
...
static void PrintTMHMMoveData(enum Item itemId)
{
    ...
    else
    {
        ...
        BagMenu_Print(WIN_TMHM_INFO, FONT_SMALL, text, 7, 12, 0, 0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);
        ...
        BagMenu_Print(WIN_TMHM_INFO, FONT_SMALL, text, 7, 24, 0, 0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);

        // Print TMHM pp
        ConvertIntToDecimalStringN(gStringVar1, GetMovePP(move), STR_CONV_MODE_RIGHT_ALIGN, 3);
        BagMenu_Print(WIN_TMHM_INFO, FONT_SMALL, gStringVar1, 7, 36, 0, 0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);
