...
{
    if (IS_FRLG)
        InitRegionMapWithExitCB(REGIONMAP_TYPE_WALL, GetRegionMap(gMapHeader.regionMapSectionId), CB2_ReturnToFieldContinueScriptPlayMapMusic);
    else
...
{
    return (MetatileBehavior_IsPC(GetMetatileAttributesById(tileId, METATILE_ATTRIBUTE_BEHAVIOR)));
}
...
static void PCTurnOnEffect_SetMetatile(s16 isScreenOn, s8 dx, s8 dy)
{
    ...
    if (isScreenOn)
    {
        ...
        if (gSpecialVar_0x8004 == PC_LOCATION_OTHER)
            metatileId = METATILE_RG_Building_PCOff;
    ...
    else
    {
        ...
        if (gSpecialVar_0x8004 == PC_LOCATION_OTHER)
            metatileId = METATILE_RG_Building_PCOn;
        else if (gSpecialVar_0x8004 == PC_LOCATION_BRENDANS_HOUSE)
...
void IsGrassTypeInParty(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (GetMonData(pokemon, MON_DATA_SANITY_HAS_SPECIES) && !GetMonData(pokemon, MON_DATA_IS_EGG))
        {
            ...
            if (GetSpeciesType(species, 0) == TYPE_NATURE || GetSpeciesType(species, 1) == TYPE_NATURE)
            {
...
{
    return GetMonData(&gParties[B_TRAINER_PLAYER][GetLeadMonIndex()], MON_DATA_NUZLOCKE_RIBBON);
}
...
void GiveLeadMonEffortRibbon(void)
{
    ...
    SetMonData(leadMon, MON_DATA_NUZLOCKE_RIBBON, &ribbonSet);
    if (GetRibbonCount(leadMon) > NUM_CUTIES_RIBBONS)
        TryPutSpotTheCutiesOnAir(leadMon, MON_DATA_NUZLOCKE_RIBBON);
}
...
{
    u16 monData;
    CreateRandomMon(&gParties[B_TRAINER_PLAYER][0], SPECIES_CHIBI_CHEN, 7);
    monData = TRUE;
...
static void UIShowMoveList(u8 taskId)
{
    ...
    if (BW_SUMMARY_SCREEN)
        ShowSelectMovePokemonSummaryScreen_BW(gParties[B_TRAINER_PLAYER], gTasks[taskId].tPartyIndex, CB2_ReturnToFieldWhileLearningMove, gTasks[taskId].tMove);
    else
