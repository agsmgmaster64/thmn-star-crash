...
static void Task_MoveRelearner_Continue_Answer(u8 taskId)
{
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
    case 1: // No
...
static void Task_MoveRelearner_Continue_Prompt(u8 taskId)
{
    ...
    MoveRelearnerCreateYesNoMenu();
    gTasks[taskId].func = Task_MoveRelearner_Continue_Answer;
}
...
static void Task_MoveRelearner_HandleInput(u8 taskId)
{
    ...
    case LIST_CANCEL:
        ...
        if (gRelearnMode == RELEARN_MODE_SCRIPT)
        {
            gTasks[taskId].func = Task_MoveRelearner_Continue_Prompt;
            StringExpandPlaceholders(gStringVar4, gText_MoveRelearnerContinue);
        }
        else
        {
...
void MoveRelearnerShowHideCategoryIcon(s32 moveId)
{
    ...
    {
        if (sMoveRelearnerStruct->categoryIconSpriteId == 0xFF)
            sMoveRelearnerStruct->categoryIconSpriteId = CreateSprite(&gSpriteTemplate_CategoryIcons, 66, 39, 0);
