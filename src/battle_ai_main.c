...
static s32 AI_CheckBadMove(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    if (moveTarget != TARGET_USER)
    {
        ...
        case ABILITY_FLOWER_VEIL:
            if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE) && (IsNonVolatileStatusMove(move)))
                RETURN_SCORE_MINUS(10);
            break;
        ...
        if (hasTwoOpponents)
        {
            ...
            case ABILITY_SWEET_VEIL:
                ...
                break;
            case ABILITY_FLOWER_VEIL:
                if ((IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE)) && (IsNonVolatileStatusMove(move) || IsStatLoweringEffect(moveEffect)))
                    RETURN_SCORE_MINUS(10);
                break;
            case ABILITY_AROMA_VEIL:
                if (IsAromaVeilProtectedEffect(moveEffect))
        ...
        if (IsHolyTerrainAffected(battlerDef, abilityDef, aiData->holdEffects[battlerAtk], gFieldStatuses))
        {
...
// the following checks apply to any target (including user)
    ...
    // stat raising effects
        ...
        case EFFECT_ROTOTILLER:
            if (hasPartner)
            {
                if (!(IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NATURE)
                  ...
                  && !(IS_BATTLER_OF_TYPE(BATTLE_PARTNER(battlerAtk), TYPE_NATURE)
            ...
            else if (!(IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NATURE)
              && AI_IsBattlerGrounded(battlerAtk)
              && (BattlerStatCanRise(battlerAtk, aiData->abilities[battlerAtk], STAT_ATK) || BattlerStatCanRise(battlerAtk, aiData->abilities[battlerAtk], STAT_SPATK))))
            {
                ADJUST_SCORE(-10);
    ...
    // other
        ...
        case EFFECT_LEECH_SEED:
            if (gBattleMons[battlerDef].volatiles.leechSeed
             || IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE)
             || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-10);
        ...
        case EFFECT_CORROSIVE_GAS:
            if (aiData->abilities[battlerDef] == ABILITY_COLLECTOR)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_INGRAIN:
        ...
        case EFFECT_FLOWER_SHIELD:
            if (!IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NATURE)
              && !(hasPartner && IS_BATTLER_OF_TYPE(BATTLE_PARTNER(battlerAtk), TYPE_NATURE)))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_LASER_FOCUS:
            if (gBattleMons[battlerDef].volatiles.laserFocus)
                ADJUST_SCORE(-10);
            else if (aiData->abilities[battlerDef] == ABILITY_SHELL_ARMOR || aiData->abilities[battlerDef] == ABILITY_GUARD_ARMOR)
                ADJUST_SCORE(-8);
            break;
        case EFFECT_SKETCH:
            if (aiData->lastUsedMove[battlerDef] == MOVE_NONE)
        ...
        case EFFECT_INVERSE_ROOM:
            if (gFieldStatuses & STATUS_FIELD_INVERSE_ROOM || PartnerMoveIsSameNoTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_GRAVITY:
        ...
        case EFFECT_SPICY_EXTRACT:
            if (battlerAtk != BATTLE_PARTNER(battlerDef)
             && (HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL)
              ...
              || aiData->abilities[battlerDef] == ABILITY_MAGIC_BARRIER
...
static s32 AI_DoubleBattle(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    if (aiData->partnerMove != MOVE_NONE && hasPartner)
    {
        ...
        case EFFECT_PERISH_SONG:
            ...
            {
                if (IsTrappingMove(aiData->partnerMove) || GetMoveEffect(predictedMove) == EFFECT_INGRAIN)
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_BEAT_UP:
                ...
                break;
            case EFFECT_SOAK:
                if (atkPartnerAbility == ABILITY_PLAY_GHOST
...
/*
        ...
        {
        case EFFECT_SKILL_SWAP:
            if (aiData->abilities[battlerAtk] == ABILITY_FRETFUL)
                ADJUST_SCORE(GOOD_EFFECT);
...
static s32 AI_CalcMoveEffectScore(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, struct AiLogicData *aiData)
{
    ...
    case EFFECT_ATTACK_SPATK_UP:    // work up
        ...
        break;
    case EFFECT_ROTOTILLER:
            if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_NATURE) && AI_IsBattlerGrounded(battlerAtk))
            {
                ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK));
                ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK));
            }
            if (hasPartner && IS_BATTLER_OF_TYPE(BATTLE_PARTNER(battlerAtk), TYPE_NATURE) && AI_IsBattlerGrounded(BATTLE_PARTNER(battlerAtk)))
            {
                ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_ATK));
                ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_SPATK));
            }
            if (IS_BATTLER_OF_TYPE(LEFT_FOE(battlerAtk), TYPE_NATURE) && AI_IsBattlerGrounded(LEFT_FOE(battlerAtk)))
            {
            ...
            if (IS_BATTLER_OF_TYPE(RIGHT_FOE(battlerAtk), TYPE_NATURE) && AI_IsBattlerGrounded(RIGHT_FOE(battlerAtk)))
            {
    ...
    case EFFECT_FLOWER_SHIELD:
        ...
        if (hasPartner && IS_BATTLER_OF_TYPE(BATTLE_PARTNER(battlerAtk), TYPE_NATURE))
        {
            ADJUST_SCORE(IncreaseStatUpScore(BATTLE_PARTNER(battlerAtk), battlerDef, STAT_CHANGE_DEF));
        }
        if (IS_BATTLER_OF_TYPE(LEFT_FOE(battlerAtk), TYPE_NATURE))
        {
            if (aiData->abilities[LEFT_FOE(battlerAtk)] == ABILITY_CONTRARY)
                ADJUST_SCORE(WEAK_EFFECT);
            else
        ...
        if (IS_BATTLER_OF_TYPE(RIGHT_FOE(battlerAtk), TYPE_NATURE))
        {
            if (aiData->abilities[RIGHT_FOE(battlerAtk)] == ABILITY_CONTRARY)
                ADJUST_SCORE(WEAK_EFFECT);
            else
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || aiData->abilities[battlerAtk] == ABILITY_LUCID_DREAMING
    ...
    case EFFECT_MIMIC:
        ...
        break;
    case EFFECT_LEECH_SEED:
        if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_NATURE)
    ...
    case EFFECT_FORESIGHT:
        if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY || aiData->abilities[battlerAtk] == ABILITY_MINDS_EYE)
         ...
         || (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GHOST)
         && (HasMoveWithType(battlerAtk, TYPE_ILLUSION)
         || HasMoveWithType(battlerAtk, TYPE_DREAM))))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_MIRACLE_EYE:
        if (gBattleMons[battlerDef].statStages[STAT_EVASION] > DEFAULT_STAT_STAGE
          || (IS_BATTLER_OF_TYPE(battlerDef, TYPE_DARK) && (HasMoveWithType(battlerAtk, TYPE_REASON))))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_PERISH_SONG:
        if (IsBattlerTrapped(battlerAtk, battlerDef))
        ...
        default:
            if (GetMoveEffect(move) != EFFECT_BESTOW && aiData->items[battlerAtk] == ITEM_NONE && aiData->items[battlerDef] != ITEM_NONE)
            {
                ...
                case HOLD_EFFECT_FROST_ORB:
                    ...
                    break;
                case HOLD_EFFECT_BLACK_SLUDGE:
                    if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_MIASMA) || aiData->abilities[battlerAtk] == ABILITY_MAGIC_GUARD)
                        ADJUST_SCORE(DECENT_EFFECT);
                    break;
                case HOLD_EFFECT_IRON_BALL:
    ...
    case EFFECT_PSYCHIC_TERRAIN:
        ...
        break;
    case EFFECT_HOLY_TERRAIN:
        if (ShouldSetFieldStatus(battlerAtk, STATUS_FIELD_HOLY_TERRAIN))
        {
            ADJUST_SCORE(GOOD_EFFECT);
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_TERRAIN_EXTENDER || HasBattlerSideMoveWithEffect(battlerAtk, EFFECT_TERRAIN_PULSE))
                ADJUST_SCORE(WEAK_EFFECT);
    ...
    case EFFECT_HEAL_BLOCK:
        if (AI_IsFaster(battlerAtk, battlerDef, move, predictedMoveSpeedCheck, CONSIDER_PRIORITY) && predictedMove != MOVE_NONE && IsHealingMove(predictedMove))
          ...
          || (aiData->holdEffects[battlerDef] == HOLD_EFFECT_BLACK_SLUDGE && IS_BATTLER_OF_TYPE(battlerDef, TYPE_MIASMA)))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_SOAK:
        ...
        break;
    case EFFECT_THIRD_TYPE:
        if (aiData->abilities[battlerDef] == ABILITY_PLAY_GHOST)
    ...
    case EFFECT_SMACK_DOWN:
        if (!AI_IsBattlerGrounded(battlerDef) && HasDamagingMoveOfType(battlerAtk, TYPE_EARTH) && !CanTargetFaintAi(battlerDef, battlerAtk))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_KNOCK_OFF:
        if (CanKnockOffItem(battlerDef, battlerAtk, aiData->items[battlerDef]))
    ...
    for (u32 effectId = 0; effectId < additionalEffectCount; effectId++)
    {
        ...
        else // consider move effects that hinder the target
        {
            ...
            case MOVE_EFFECT_BUG_BITE:   // And pluck
                if (gBattleMons[battlerDef].volatiles.substitute || aiData->abilities[battlerDef] == ABILITY_COLLECTOR)
                    break;
            ...
            case MOVE_EFFECT_REMOVE_STATUS:
                if (gBattleMons[battlerDef].status1 & GetMoveEffectArg_Status(move))
                {
                    if (ShouldCureStatus(battlerAtk, battlerDef, aiData))
                        ADJUST_SCORE(DECENT_EFFECT);
                    else if (aiData->holdEffects[battlerDef] == HOLD_EFFECT_FLAME_ORB || aiData->holdEffects[battlerDef] == HOLD_EFFECT_TOXIC_ORB || aiData->holdEffects[battlerDef] == HOLD_EFFECT_FROST_ORB)
                        ADJUST_SCORE(WEAK_EFFECT);
                    else
                        ADJUST_SCORE(BAD_EFFECT);
                }
...
static s32 AI_PowerfulStatus(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    case EFFECT_WONDER_ROOM:
        ...
        break;
    case EFFECT_INVERSE_ROOM:
        if (!(gFieldStatuses & STATUS_FIELD_INVERSE_ROOM) && !HasMoveWithEffect(battlerDef, EFFECT_INVERSE_ROOM))
            ADJUST_SCORE(POWERFUL_STATUS_MOVE);
        break;
    case EFFECT_GRAVITY:
        if (!(gFieldStatuses & STATUS_FIELD_GRAVITY))
