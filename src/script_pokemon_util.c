...
void CanHyperTrain(struct ScriptContext *ctx)
{
    ...
    if (GetMonData(&gParties[B_TRAINER_PLAYER][partyIndex], MON_DATA_HP_IV + stat) < MAX_PER_STAT_IVS)
    {
        gSpecialVar_Result = TRUE;
    }
    else
...
void CanUnhyperTrain(struct ScriptContext *ctx)
{
    ...
    if (GetMonData(&gParties[B_TRAINER_PLAYER][partyIndex], MON_DATA_HP_IV + stat) > 0)
    {
        gSpecialVar_Result = TRUE;
    }
    else
...
{
    return ScriptGiveMonParameterized(B_SIDE_PLAYER, PARTY_SIZE, species, level, item, ball, nature, abilityNum, gender, evs, ivs, moves, shinyMode, ggMaxFactor, teraType, 0, 0);
}
