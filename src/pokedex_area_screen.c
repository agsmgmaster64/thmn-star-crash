...
static const enum Species sSpeciesHiddenFromAreaScreen[] = { SPECIES_NORMAL_KOSUZU };
...
{
    //{SPECIES_CHIBI_WAKASAGIHIME_GOLD, MAP_GROUP(MAP_ROUTE119), MAP_NUM(MAP_ROUTE119)},
    {NUM_SPECIES}
};
...
{
    u8 currentMapGroup = gWildMonHeaders[headerSectionId].mapGroup;
    u8 currentMapNum = gWildMonHeaders[headerSectionId].mapNum;
    ...
    if (MonListHasSpecies(info, species, currentMapGroup, currentMapNum, WILD_AREA_LAND))
        return TRUE;
    if (MonListHasSpecies(info, species, currentMapGroup, currentMapNum, WILD_AREA_WATER))
        return TRUE;
    if (MonListHasSpecies(info, species, currentMapGroup, currentMapNum, WILD_AREA_FISHING))
        return TRUE;
...
static bool8 MonListHasSpecies(const struct WildEncounterTypes *info, enum Species species, u32 currentMapGroup, u32 currentMapNum, enum WildPokemonArea area)
{
    ...
    if (actualInfo != NULL)
    {
        ...
        {
            u16 curSpecies;
            curSpecies = actualInfo->wildPokemon[i].species;
            #if RANDOMIZER_AVAILABLE == TRUE
                if (!IsRandomizationPossible(curSpecies, species))
                    continue;
                curSpecies = RandomizeWildEncounter(
                    curSpecies, currentMapGroup, currentMapNum, area, i);
            #endif
...
static void Task_HandlePokedexAreaScreenInput(u8 taskId)
{
    ...
    case 1:
        ...
        else if (JOY_NEW(DPAD_LEFT) || (JOY_NEW(L_BUTTON) && gSaveBlock2Ptr->optionsLButtonMode != OPTIONS_L_BUTTON_MODE_L_EQUALS_A))
        {
            gTasks[taskId].data[1] = 1;
            PlaySE(SE_DEX_PAGE);
        }
        else if (JOY_NEW(DPAD_RIGHT) || (JOY_NEW(R_BUTTON) && gSaveBlock2Ptr->optionsLButtonMode != OPTIONS_L_BUTTON_MODE_L_EQUALS_A))
        {
