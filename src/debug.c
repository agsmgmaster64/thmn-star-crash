...
#define DEBUG_NUMBER_DIGITS_VIAL 3
...
static const u8 *const gFollowerNPCStringsTable[DEBUG_FNPC_COUNT] = {
    ...
    COMPOUND_STRING("Gigi"),
    COMPOUND_STRING("Kronii"),
};
...
static const struct DebugMenuOption sDebugMenu_Actions_FollowerNPCMenu_Create[] =
{
    ...
    [DEBUG_FNPC_GIGI] =    { gFollowerNPCStringsTable[DEBUG_FNPC_GIGI],    DebugAction_CreateFollowerNPC },
    [DEBUG_FNPC_KRONII] =  { gFollowerNPCStringsTable[DEBUG_FNPC_KRONII],  DebugAction_CreateFollowerNPC },
    { NULL }
};
...
static const struct DebugMenuOption sDebugMenu_Actions_EditPokemon[] =
{
    ...
    { COMPOUND_STRING("Set Tera Type"),      DebugAction_ExecuteScript, Debug_EventScript_SetTeraType },
    { NULL }
};
...
static const struct DebugMenuOption sDebugMenu_Actions_Flags[] =
{
    ...
    [DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_TERASTALIZE]   = { COMPOUND_STRING("Toggle {STR_VAR_1}Terastal"),        DebugAction_ToggleFlag, DebugAction_FlagsVars_ToggleTerastalize },
...
static u32 Debug_CheckToggleFlags(u8 id)
{
    ...
    case DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_TERASTALIZE:
        result = B_FLAG_TERA_ORB_CHARGED ? FlagGet(B_FLAG_TERA_ORB_CHARGED) : DEBUG_OPTION_CANT_BE_TOGGLED;
        break;
...
static void Debug_Display_OutfitInfo(u32 outfit, u32 digit, u8 windowId)
{
    ...
    if (GetOutfitStatus(outfit))
        ...
        StringCopyPadded(gStringVar2, sDebugText_False, CHAR_SPACE, 15);
    StringCopy(gStringVar3, gText_DigitIndicator[digit]);
    StringExpandPlaceholders(gStringVar4, COMPOUND_STRING("Outfit: {STR_VAR_1}{CLEAR_TO 90}\n{STR_VAR_2}{CLEAR_TO 90}\n{STR_VAR_3}"));
    AddTextPrinterParameterized(windowId, DEBUG_MENU_FONT, gStringVar4, 0, 0, 0, NULL);
}
...
static void DebugAction_PCBag_Fill_PCBoxes_Fast(u8 taskId) //Credit: Sierraffinity
{
    ...
    enum Species species = SPECIES_CHIBI_REIMU;
...
static const u32 gDebugFollowerNPCGraphics[] =
{
    ...
    OBJ_EVENT_GFX_RED,
    OBJ_EVENT_GFX_LEAF,
    OBJ_EVENT_GFX_GIGI_MURIN,
    OBJ_EVENT_GFX_KRONII,
};
...
void DebugNative_GetAbilityNames(void)
{
    ...
    u32 cantRandomize = GetMonData(&gParties[B_TRAINER_0][gSpecialVar_0x8004], MON_DATA_CANT_RANDOMIZE_ABILITY);
...
{
    ZeroPlayerPartyMons();
    CreateNPCTrainerPartyFromTrainer(gParties[B_TRAINER_0], &sDebugTrainers[DIFFICULTY_NORMAL][DEBUG_TRAINER_PLAYER], TRUE, BATTLE_TYPE_TRAINER, 0);
    ScriptContext_Enable();
    Debug_DestroyMenu_Full(taskId);
}
...
static void DebugAction_Party_BattleSingle(u8 taskId)
{
    ...
    ZeroEnemyPartyMons();
    CreateNPCTrainerPartyFromTrainer(gParties[B_TRAINER_0], &sDebugTrainers[DIFFICULTY_NORMAL][DEBUG_TRAINER_PLAYER], TRUE, BATTLE_TYPE_TRAINER, 0);
    CreateNPCTrainerPartyFromTrainer(gParties[B_TRAINER_1], GetDebugAiTrainer(), FALSE, BATTLE_TYPE_TRAINER, 0);
