...
static u8 ShowCategoryIcon(enum DamageCategory category)
{
    ...
    if (sMonSummaryScreen->categoryIconSpriteId == 0xFF)
        sMonSummaryScreen->categoryIconSpriteId = CreateSprite(&gSpriteTemplate_CategoryIcons, 48, 127, 0);
...
{
    int xPos;
    if (!InBattleFactory() && !InSlateportBattleTent() && !IsEnemyMon() && BXPY_IsSummaryScreenForEnemy(sMonSummaryScreen->mode) != TRUE)
    {
...
static void BufferMonTrainerMemo(void)
{
    ...
    if (InBattleFactory()  || InSlateportBattleTent()  || IsInGamePartnerMon()  || BXPY_IsSummaryScreenForEnemy(sMonSummaryScreen->mode) || IsEnemyMon())
    {
