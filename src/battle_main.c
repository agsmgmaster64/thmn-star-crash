...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_BUG_MANIAC] = { _("Fairy Maniac"), 15 },
    ...
    [TRAINER_CLASS_SR_AND_JR] = { _("Teammates"), 4 },
    ...
    [TRAINER_CLASS_TRIATHLETE] = { _("Triathelete"), 10 },
    ...
    [TRAINER_CLASS_BUG_CATCHER] = { _("Fairy Hunter"), 4 },
    ...
    [TRAINER_CLASS_HOLOLIVE_MEMBER] = { _("Hololive"), 25, BALL_LUXURY },
    [TRAINER_CLASS_THE_GUY] = { _("The"), 10, BALL_FOUL },
    [TRAINER_CLASS_MISCHIEVOUS_FAIRY] = { _("Mischievous"), 4 },
    [TRAINER_CLASS_SUNFLOWER_FAIRY] = { _("Sunflower"), 7 },
...
void CreateTrainerPartyForPlayer(void)
{
    ...
    CreateNPCTrainerPartyFromTrainer(gParties[B_TRAINER_0], GetTrainerStructFromId(gSpecialVar_0x8004), TRUE, BATTLE_TYPE_TRAINER, gSpecialVar_0x8004);
}
...
void SpriteCB_OpponentMonFromBall(struct Sprite *sprite)
{
    if (sprite->affineAnimEnded)
    {
        ...
        {
            if (HasTwoFramesAnimation(sprite->sSpeciesId) && gSaveBlock2Ptr->optionsMonAnimations == OPTIONS_GENERAL_ON)
...
void SpriteCB_PlayerMonSlideIn(struct Sprite *sprite)
{
    ...
    else if (sprite->data[3] == 1)
    {
        if (sprite->animEnded)
            return;
        sprite->data[4] = sprite->x; //sprite->data[4] = sprite->data[5];
        sprite->x = -33;
...
static void BattleStartClearSetData(void)
{
    ...
    gBattleStruct->safariEscapeFactor = gSpeciesInfo[GetMonData(&gParties[B_TRAINER_1][0], MON_DATA_SPECIES)].safariZoneFleeRate * 100 / 1275;
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!IsBattleControllerActiveOrPendingSyncAnywhere(battler))
            {
                ...
                case B_ACTION_SWITCH:
                    ...
                    if (gBattleTypeFlags & BATTLE_TYPE_ARENA
                        || (!CanBattlerEscape(battler) && GetBattlerHoldEffect(battler) != HOLD_EFFECT_SHED_SHELL && GetBattlerAbility(battler) != ABILITY_RUN_AWAY))
                    {
...
u32 GetBattlerTotalSpeedStat(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE || holdEffect == HOLD_EFFECT_POWER_ITEM)
        ...
        speed = (speed * 150) / 100;
    else if (holdEffect == HOLD_EFFECT_QUICK_POWDER && gBattleMons[battler].species == SPECIES_KEDAMA && !(gBattleMons[battler].volatiles.transformed))
        speed *= 2;
...
s32 GetBattleMovePriority(enum BattlerId battler, enum Ability ability, enum Move move)
{
    ...
    else if (ability == ABILITY_CLEAR_VOICE && IsSoundMove(move))
    {
        priority++;
    }
    ...
    case EFFECT_REVELATION_DANCE:
        if (gimmick != GIMMICK_Z_MOVE)
        {
            ...
            if (gimmick == GIMMICK_TERA && ((teraType = GetMonData(mon, MON_DATA_TERA_TYPE)) != TYPE_STELLAR))
                return teraType;
            else if (types[0] != TYPE_NONE && !(gBattleMons[battler].volatiles.roostActive && types[0] == TYPE_FLYING))
                return types[0];
            else if (types[1] != TYPE_NONE && !(gBattleMons[battler].volatiles.roostActive && types[1] == TYPE_FLYING))
                return types[1];
            else if (gBattleMons[battler].volatiles.roostActive)
                return (B_ROOST_PURE_FLYING >= GEN_5 ? TYPE_ILLUSION : TYPE_NONE);
            else if (types[2] != TYPE_NONE)
                return types[2];
            else
                return TYPE_NONE;
        }
