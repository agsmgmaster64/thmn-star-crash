...
static enum CancelerResult CancelerRecharge(struct BattleCalcValues *cv)
{
    ...
    {
        if (GetConfig(B_TRUANT) >= GEN_5 && cv->abilities[cv->battlerAtk] == ABILITY_FRETFUL)
...
{
    if (cv->abilities[cv->battlerAtk] != ABILITY_FRETFUL)
        return CANCELER_RESULT_SUCCESS;
    ...
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        ...
        case EFFECT_STRENGTH_SAP:
            if (gBattleStruct->passiveHpUpdate[cv->battlerAtk] > 0 && !IsBattlerUnaffectedByMove(battlerDef))
            {
                ...
                if (cv->abilities[battlerDef] == ABILITY_STRANGE_MIST)
                {
...
static enum MoveEndResult MoveEndUpdateLastMoves(struct BattleCalcValues *cv)
{
     ...
     && originalEffect != EFFECT_LUNAR_DANCE)
    {
        ...
        while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
        {
            ...
            {
                gLastHitByType[battlerDef] = TYPE_NONE;
    ...
    while (gBattleStruct->eventState.moveEndBattler < gBattlersCount)
    {
        ...
        case EFFECT_KNOCK_OFF:
             ...
             && !NoAliveMonsForEitherParty())
            {
                ...
                if (cv->abilities[battlerDef] == ABILITY_COLLECTOR)
                {
        ...
        case EFFECT_HIT_SWITCH_TARGET:
            ...
            {
                 if (cv->abilities[battlerDef] == ABILITY_GATE_KEEPER)
                {
                    gBattlerAbility = battlerDef;
