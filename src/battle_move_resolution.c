...
static enum Move GetDemonBookMove(struct BattleCalcValues *cv);
...
static enum CancelerResult CancelerAsleepOrFrozen(struct BattleCalcValues *cv)
{
    ...
    if (gBattleMons[cv->battlerAtk].status1 & STATUS1_SLEEP)
    {
        ...
        else
        {
            ...
            if (gBattleMons[cv->battlerAtk].status1 & STATUS1_SLEEP)
            {
                if (IsAbilityAndRecord(cv->battlerAtk, cv->abilities[cv->battlerAtk], ABILITY_LUCID_DREAMING))
                {
                    gBattlerAbility = cv->battlerAtk;
                    BattleScriptCall(BattleScript_MoveUsedLucidDreaming);
                    result = CANCELER_RESULT_RUN_SCRIPT_AND_INCREMENT;
                }
                else if (cv->moveEffect == EFFECT_SNORE)
                {
...
static enum CancelerResult CancelerPowerPoints(struct BattleCalcValues *cv)
{
    if (gBattleMons[cv->battlerAtk].pp[gCurrMovePos] == 0
     ...
     && !gSpecialStatuses[cv->battlerAtk].concertoUsedMove
...
{
    if (GetBattlerAbility(cv->battlerAtk) == ABILITY_FRETFUL && gBattleMons[cv->battlerAtk].volatiles.truantCounter)
    {
        CancelMultiTurnMoves(cv->battlerAtk);
...
static enum CancelerResult CancelerChoiceLock(struct BattleCalcValues *cv)
{
    ...
    if (gChosenMove != MOVE_STRUGGLE
     ...
     && (IsHoldEffectChoice(holdEffect) || cv->abilities[cv->battlerAtk] == ABILITY_ONI_TACTICS))
...
static enum CancelerResult CancelerCallSubmove(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_ME_FIRST:
        ...
        break;
    case EFFECT_DEMON_BOOK:
        calledMove = GetDemonBookMove(cv);
        break;
    default:
        noEffect = TRUE;
        break;
        ...
        if (gBattleMons[cv->battlerAtk].status1 & STATUS1_SLEEP
         || cv->abilities[cv->battlerAtk] == ABILITY_COMATOSE)
            ...
            battleScript = BattleScript_AlreadyAtFullHp;
              ...
              || cv->abilities[cv->battlerAtk] == ABILITY_LIFESTREAM)
...
static enum MoveEndResult MoveEndQueueConcerto(struct BattleCalcValues *cv)
{
    if (!IsSoundMove(cv->move)
     || IsBattlerUnaffectedByMove(cv->battlerDef)
     || gBattleStruct->unableToUseMove
     || gSpecialStatuses[cv->battlerAtk].concertoUsedMove
     || gBattleStruct->snatchedMoveIsUsed
    ...
    for (enum BattlerId battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        if (cv->abilities[battler] == ABILITY_CONCERTO)
            gBattleMons[battler].volatiles.activateConcerto = TRUE;
    }
...
static enum MoveEndResult MoveEndMoveBlock(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_KNOCK_OFF:
         ...
         && !NoAliveMonsForEitherParty())
        {
            ...
            if (cv->abilities[cv->battlerDef] == ABILITY_COLLECTOR)
            {
                gBattlerAbility = cv->battlerDef;
            ...
            if (gBattleMons[cv->battlerDef].ability != ABILITY_ONI_TACTICS)
    ...
    case EFFECT_HIT_SWITCH_TARGET:
        ...
        {
            if (cv->abilities[cv->battlerDef] == ABILITY_GATE_KEEPER)
            {
     ...
     && !GetBattlerPartyState(cv->battlerAtk)->isKnockedOff) // Gen3 edge case where the knocked of item was not removed
    {
        ...
        for (u32 i = 0; i < gBattlersCount; i++)
        {
              ...
              && CanStealItem(battlerDef, cv->battlerAtk, gBattleMons[cv->battlerAtk].item))
            {
                ...
                if (cv->abilities[cv->battlerAtk] != ABILITY_COLLECTOR)
...
static enum MoveEndResult MoveEndConcerto(struct BattleCalcValues *cv)
{
    enum MoveEndResult result = MOVEEND_RESULT_CONTINUE;

    if (AbilityBattleEffects(ABILITYEFFECT_CONCERTO, cv->battlerAtk, ABILITY_CONCERTO, cv->move, TRUE))
        result = MOVEEND_RESULT_RUN_SCRIPT;
...
enum Move GetNaturePowerMove(void)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
        ...
        move = MOVE_ENERGY_LIGHT;
    else if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
        move = MOVE_MANA_BURST;
...
static enum Move GetDemonBookMove(struct BattleCalcValues *cv)
{
    ...
    dmgCtx.abilities[dmgCtx.battlerAtk] = cv->abilities[cv->battlerAtk];
    dmgCtx.abilities[dmgCtx.battlerDef] = cv->abilities[cv->battlerDef];
