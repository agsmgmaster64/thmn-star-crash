...
bool8 MEScrCmd_givepokemon(struct ScriptContext *ctx)
{
    ...
    {
        memcpy(&gParties[B_TRAINER_0][PARTY_SIZE - 1], pokemonPtr, sizeof(struct Pokemon));
        ...
        heldItem = GetMonData(&gParties[B_TRAINER_0][PARTY_SIZE - 1], MON_DATA_HELD_ITEM);
        if (ItemIsMail(heldItem))
            GiveMailToMon(&gParties[B_TRAINER_0][PARTY_SIZE - 1], &mail);
        CompactPartySlots();
