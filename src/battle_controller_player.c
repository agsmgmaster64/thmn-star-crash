...
static enum TrainerPicID PlayerGetTrainerBackPicId(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
        ...
        trainerPicId = GetPlayerTrainerPicIdByOutfitGenderType(gSaveBlock3Ptr->currOutfitId, gSaveBlock2Ptr->playerGender, 1);
...
static void PlayerHandleDrawTrainerPic(enum BattlerId battler)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER && gPartnerTrainerId < TRAINER_PARTNER(PARTNER_NONE))
    {
        trainerPicId = GetPlayerTrainerPicIdByOutfitGenderType(gSaveBlock3Ptr->currOutfitId, gSaveBlock2Ptr->playerGender, 0);
        isFrontPic = TRUE;
    }
