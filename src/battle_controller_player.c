...
static void HandleInputChooseAction(enum BattlerId battler)
{
    ...
    if (JOY_REPEAT(DPAD_ANY) && gSaveBlock2Ptr->optionsLButtonMode == OPTIONS_L_BUTTON_MODE_L_EQUALS_A)
...
void HandleInputChooseTarget(enum BattlerId battler)
{
    ...
    if (JOY_HELD(DPAD_ANY) && gSaveBlock2Ptr->optionsLButtonMode == OPTIONS_L_BUTTON_MODE_L_EQUALS_A)
        gPlayerDpadHoldFrames++;
    else
        gPlayerDpadHoldFrames = 0;
...
static void ReloadMoveNames(enum BattlerId battler)
{
    ...
    {
        BattleUI_DisplayMoveBox(CheckTargetTypeEffectiveness(battler), battler);
        return;
    }
    // end bwBattleUI
...
static void MoveSelectionDisplayMoveType(enum BattlerId battler)
{
    ...
    {
        type = TYPE_ILLUSION; // Max Guard is always a Normal-type move
    }
...
enum TrainerPicID LinkPlayerGetTrainerPicId(u32 multiplayerId)
{
    ...
    if (version != VERSION_RUBY && version != VERSION_SAPPHIRE
     && outfitId < OUTFIT_COUNT)
        return GetPlayerTrainerPicIdByOutfitGenderType(outfitId, gender);
...
static void PlayerHandleDrawTrainerPic(enum BattlerId battler)
{
    ...
    if (TESTING)
    {
        trainerPicId = TRAINER_PIC_RENKO;
...
const u8 *BattleUI_GetTypeEffectivenessSymbol(u32 effectiveness)
{
    ...
    case EFFECTIVENESS_EXTREMELY_EFFECTIVE:
        return extremelyEffectiveIcon;
    case EFFECTIVENESS_MOSTLY_INEFFECTIVE:
        return mostlyIneffectiveIcon;
...
static void MoveSelectionDisplayMoveEffectiveness(u32 foeEffectiveness, enum BattlerId battler)
{
    ...
    if (BW_BATTLE_UI && BW_BATTLE_UI_TEXTBOX && BW_BATTLE_UI_INPUTBOX)
    {
        BattleUI_DisplayMoveBox(foeEffectiveness, battler);
        return;
    }
