...
static const u8 sText_Stats_Gender_0[] = _("{YIN}");
static const u8 sText_Stats_Gender_12_5[] = _("{YIN} 7/1 {YANG}"); //_("♀ 12.5 / 87.5 ♂");
static const u8 sText_Stats_Gender_25[] = _("{YIN} 3/1 {YANG}");     //_("♀ 25 / 75 ♂");
static const u8 sText_Stats_Gender_50[] = _("{YIN} / {YANG}");     //_("♀ 50 / 50 ♂");
static const u8 sText_Stats_Gender_75[] = _("{YIN} 1/3 {YANG}");     //_("♀ 75 / 25 ♂");
static const u8 sText_Stats_Gender_87_5[] = _("{YIN} 1/7 {YANG}");
static const u8 sText_Stats_Gender_100[] = _("{YANG}");
...
#define WIN_STATS_NAME_GENDER 1
#define WIN_STATS_LEFT 2
#define WIN_STATS_NAVIGATION_BUTTONS 3
#define WIN_STATS_MOVES_TOP 4
#define WIN_STATS_MOVES_DESCRIPTION 5
...
static const struct WindowTemplate sStatsScreen_WindowTemplates[] =
{
    ...
    [WIN_STATS_ABILITIES] =
    {
        ...
        .height = 8,
        .paletteNum = 0,
        .baseBlock = 1 + 60 + 48 + 96 + 24 + 72 + 108,
    },
    [WIN_STATS_LEFT_UNUSED] =
    {
        ...
        .height = 4,
        .paletteNum = 0,
        .baseBlock = 1 + 60 + 48 + 96 + 24 + 72 + 108 + 144,
    },
    DUMMY_WIN_TEMPLATE
};
...
static const struct SearchOptionText sDexSearchColorOptions[] =
{
    ...
    [SOURCE_VTUBERS + 1]      = {gText_DexOtherDescription,   gText_DexSearchSourceVTubers},
...
static const struct SearchOptionText sDexSearchTypeOptions[] =
{
    ...
    {gText_DexEmptyString, gTypesInfo[TYPE_MIASMA].name},
    {gText_DexEmptyString, gTypesInfo[TYPE_EARTH].name},
    {gText_DexEmptyString, gTypesInfo[TYPE_BEAST].name},
    {gText_DexEmptyString, gTypesInfo[TYPE_HEART].name},
    ...
    {gText_DexEmptyString, gTypesInfo[TYPE_NATURE].name},
    {gText_DexEmptyString, gTypesInfo[TYPE_ELECTRIC].name},
    {gText_DexEmptyString, gTypesInfo[TYPE_REASON].name},
...
static void Task_HandleInfoScreenInput(u8 taskId)
{
    ...
    if ((JOY_NEW(DPAD_RIGHT) || (JOY_NEW(R_BUTTON) && gSaveBlock2Ptr->optionsLButtonMode != OPTIONS_L_BUTTON_MODE_L_EQUALS_A)))
    {
        sPokedexView->selectedScreen = AREA_SCREEN;
...
static u8 ShowCategoryIcon(enum DamageCategory category)
{
    if (sPokedexView->categoryIconSpriteId == 0xFF)
        sPokedexView->categoryIconSpriteId = CreateSprite(&gSpriteTemplate_CategoryIcons, 174, 43, 0);
...
static void Task_HandleStatsScreenInput(u8 taskId)
{
    ...
    //Switch screens
    if ((JOY_NEW(DPAD_LEFT) || (JOY_NEW(L_BUTTON) && gSaveBlock2Ptr->optionsLButtonMode != OPTIONS_L_BUTTON_MODE_L_EQUALS_A)))
    {
...
static void PrintStatsScreen_Moves_Description(u8 taskId)
{
    ...
    case 0:
        ...
        PrintStatsScreenTextSmall(WIN_STATS_MOVES_DESCRIPTION, gStringVar1, moves_x + 30, moves_y);
        //Physical/Special/Status Category
        DestroyCategoryIcon();
        ...
        PrintStatsScreenTextSmall(WIN_STATS_MOVES_DESCRIPTION, gStringVar1,  moves_x + 79, moves_y);
        //PP
...
static void PrintStatsScreen_Abilities(u8 taskId)
{
    ...
    if (abilityChosen != ABILITY_NONE)
    {
        ...
        abilityDesc = gAbilitiesInfo[abilityChosen].descriptionLong;
        if (abilityDesc == NULL)
        {
            abilityDesc = gAbilitiesInfo[abilityChosen].description;
        }
        FormatTextByWidth(gStringVar2, 134, FONT_SMALL, abilityDesc, 0);
        ...
        else
        {
            ...
            for (u32 j = 0; evolutions[i].params != NULL && evolutions[i].params[j].condition != CONDITIONS_END; j++)
            {
                ...
                case IF_NOT_REGION:
                {
                    ...
                    case REGION_SANNI: StringAppend(gStringVar4, COMPOUND_STRING("Sanni")); break;
                    }
                    break;
                }
                // Gen 8
...
static void Task_HandleCryScreenInput(u8 taskId)
{
    ...
    else if (!gPaletteFade.active)
    {
        ...
        if (JOY_NEW(DPAD_LEFT)
         || (JOY_NEW(L_BUTTON) && gSaveBlock2Ptr->optionsLButtonMode != OPTIONS_L_BUTTON_MODE_L_EQUALS_A))
        {
            BeginNormalPaletteFade(PALETTES_ALL & ~(0x14), 0, 0, 0x10, RGB_BLACK);
            m4aMPlayContinue(&gMPlayInfo_BGM);
            sPokedexView->screenSwitchState = 2;
        ...
        if (JOY_NEW(DPAD_RIGHT)
         || (JOY_NEW(R_BUTTON) && gSaveBlock2Ptr->optionsLButtonMode != OPTIONS_L_BUTTON_MODE_L_EQUALS_A))
        {
            if (!sPokedexListItem->owned)
            {
                PlaySE(SE_FAILURE);
