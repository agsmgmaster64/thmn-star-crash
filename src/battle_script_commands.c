...
void (*const gBattleScriptingCommandsTable[])(void) =
{
    ...
    [B_SCR_OP_PURSUITDOUBLES]                        = Cmd_unused236,
...
static u32 UpdateEffectivenessResultFlagsForDoubleSpreadMoves(u32 resultFlags)
{
    ...
    for (u32 sound = 0; sound < 3; sound++)
    {
        for (enum BattlerId battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
        {
            ...
            {
            case 0:
                if (gBattleStruct->moveResultFlags[battlerDef] & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE)
                 && !DoesBattlerNegateDamage(battlerDef))
                    return gBattleStruct->moveResultFlags[battlerDef];
                break;
            case 1:
                if (gBattleStruct->moveResultFlags[battlerDef] & (MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE)
                 && !DoesBattlerNegateDamage(battlerDef))
                    return gBattleStruct->moveResultFlags[battlerDef];
                break;
            case 2:
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if ((index = IsFlowerVeilProtected(battler)) && !flags.certain)
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
                ...
                || (battlerAbility == ABILITY_HIGH_STRENGTH && statId == STAT_ATK)
...
void ApplyExperienceMultipliers(s32 *expAmount, u8 expGetterMonId, u8 faintedBattler)
{
    ...
    if (B_AFFECTION_MECHANICS == TRUE && GetMonAffectionHearts(&gPlayerParty[expGetterMonId]) >= AFFECTION_TWO_HEARTS)
        *expAmount = (*expAmount * 4915) / 4096;
