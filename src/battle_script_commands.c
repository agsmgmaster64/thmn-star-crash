...
void (*const gBattleScriptingCommandsTable[])(void) =
{
    ...
    [B_SCR_OP_WAITFANFARE]                           = Cmd_waitfanfare,
...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_CHROME_HEART,    {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } },
    { ITEM_JUST_HEART,      {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } },
    ...
    { ITEM_BLUE_UFO,        {   _,   _,   1,   4,   4,   4,   4,   5,   4,   5, } },
...
static u32 UpdateEffectivenessResultFlagsForDoubleSpreadMoves(u32 resultFlags, u32 moveTarget)
{
    ...
    for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (!(gBattleStruct->moveResultFlags[battlerDef] & MOVE_RESULT_MOSTLY_INEFFECTIVE))
            ret &= ~MOVE_RESULT_MOSTLY_INEFFECTIVE; // any battler with 1x or better effectiveness removes NVE sound
        if (gBattleStruct->moveResultFlags[battlerDef] & MOVE_RESULT_EXTREMELY_EFFECTIVE)
            ret |= MOVE_RESULT_EXTREMELY_EFFECTIVE; // any super effective result will play SE_SUPER_EFFECTIVE
    }
    return ret;
}
...
static void MoveDamageDataHpUpdate(enum BattlerId battler, u32 scriptBattler, const u8 *nextInstr)
{
    ...
    else if (DoesDisguiseBlockMove(battler, gCurrentMove) || DoesIceFaceBlockMove(battler, gCurrentMove))
    {
        ...
        gBattleStruct->moveResultFlags[battler] &= ~(MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE);
...
static void Cmd_effectivenesssound(void)
{
    ...
    if (!(moveResultFlags & MOVE_RESULT_MISSED))
    {
        ...
        default:
            if (moveResultFlags & (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE))
            {
                #if TESTING
                if (gTestRunnerEnabled)
                    TestRunner_Battle_RecordEffectivenessSound(gBattlerTarget, SE_SUPER_EFFECTIVE);
            ...
            else if (moveResultFlags & (MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE))
            {
                #if TESTING
                if (gTestRunnerEnabled)
                    TestRunner_Battle_RecordEffectivenessSound(gBattlerTarget, SE_NOT_EFFECTIVE);
...
void StealTargetItem(enum BattlerId battlerStealer, enum BattlerId itemBattler)
{
    ...
    if (GetBattlerAbility(itemBattler) != ABILITY_ONI_TACTICS)
...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_THRASH:
        ...
        if (gSpecialStatuses[effectBattler].dancerUsedMove
         || gSpecialStatuses[effectBattler].concertoUsedMove
         || gSpecialStatuses[effectBattler].wallMasterUsedMove
         || gBattleMons[effectBattler].volatiles.rampageTurns)
        {
            gBattlescriptCurrInstr = battleScript;
        }
        else
    ...
    case MOVE_EFFECT_LEECH_SEED:
        if (!IS_BATTLER_OF_TYPE(effectBattler, TYPE_NATURE) && !gBattleMons[effectBattler].volatiles.leechSeed)
        {
...
static void Cmd_jumpifcantswitch(void)
{
    ...
    enum BattlerId battler = GetBattlerForBattleScript(cmd->battler);
    if (!cmd->ignoreEscapePrevention && !CanBattlerEscape(battler) && GetBattlerHoldEffect(battler) != HOLD_EFFECT_SHED_SHELL && GetBattlerAbility(battler) != ABILITY_RUN_AWAY)
    {
        gBattlescriptCurrInstr = cmd->jumpInstr;
    }
    else
...
static void Cmd_tryswapitems(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (GetBattlerAbility(gBattlerTarget) != ABILITY_ONI_TACTICS)
                gBattleStruct->choicedMove[gBattlerTarget] = MOVE_NONE;

            if (GetBattlerAbility(gBattlerAttacker) != ABILITY_ONI_TACTICS
...
static void Cmd_settypetoenvironment(void)
{
    ...
    case STATUS_FIELD_GRASSY_TERRAIN:
        ...
        break;
    case STATUS_FIELD_MISTY_TERRAIN:
        environmentType = TYPE_HEART;
        break;
    case STATUS_FIELD_PSYCHIC_TERRAIN:
        environmentType = TYPE_REASON;
        break;
    case STATUS_FIELD_HOLY_TERRAIN:
        environmentType = TYPE_FAITH;
        break;
    default:
...
static void ComputeBallData(u32 wildMonBattler, u32 playerBattler, struct BallData *ball)
{
    ...
    if (gSpeciesInfo[battleMon->species].isSpawn)
    {
    ...
    case BALL_MASTER:
        ...
        break;
    case BALL_NET:
        if (IS_BATTLER_ANY_TYPE(wildMonBattler, TYPE_WATER, TYPE_BEAST))
...
static void Cmd_handleballthrow(void)
{
     ...
     || FlagGet(WE_FLAG_NO_CATCHING))
    {
        ...
        MarkBattlerForControllerExec(gBattlerAttacker);
        gBattlescriptCurrInstr = BattleScript_MonBallDodge;
    }
...
static void UpdatePokeFlutePartyStatus(struct Pokemon* party, enum BattlerPosition position)
{
    s32 i;
    enum BattlerId battler;
    u32 monToCheck, status, cantRandomizeAbility;
    enum Species species;
    u16 abilityNum;
    monToCheck = 0;
    for (i = 0; i < PARTY_SIZE; i++)
...
void BS_HandleBallCatchEffects(void)
{
    NATIVE_ARGS();
    ...
    if (IsVictoryCatch())
        gBattlescriptCurrInstr = cmd->nextInstr;
    else if (ballId == BALL_BOMB)
        gBattlescriptCurrInstr = BattleScript_BombOrbDamage;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
void BS_PlayTrainerVictoryMusic(void)
{
    NATIVE_ARGS();
