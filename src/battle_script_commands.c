...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_INCINERATE:
        if (((gBattleMons[gEffectBattler].item >= FIRST_BERRY_INDEX && gBattleMons[gEffectBattler].item <= LAST_BERRY_INDEX)
         ...
         && abilities[gEffectBattler] != ABILITY_COLLECTOR)
        {
    ...
    case MOVE_EFFECT_LEECH_SEED:
        if (!IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_NATURE) && !gBattleMons[gBattlerTarget].volatiles.leechSeed)
        {
...
void BS_TryFlingHoldEffect(void)
{
    ...
    case HOLD_EFFECT_ICY_BALL:
        SetMoveEffect(gBattlerAttacker, gBattlerTarget, MOVE_EFFECT_FROSTBITE, cmd->nextInstr, NO_FLAGS);
        break;
    case HOLD_EFFECT_TYPE_POWER:
        if (GetItemSecondaryId(gBattleStruct->flingItem) != TYPE_MIASMA)
            gBattlescriptCurrInstr = cmd->nextInstr;
        else
