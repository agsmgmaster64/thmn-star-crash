...
{
    if (GetMonData(mon, MON_DATA_IS_EGG) != TRUE && GetMonData(mon, MON_DATA_SPECIES) == SPECIES_NORMAL_YUYUKO)
        return TRUE;
    return FALSE;
}
...
static void BlitBitmapToPartyWindow_Equal(u8 windowId, u8 x, u8 y, u8 width, u8 height, bool8 hideHP)
{
    ...
    if (hideHP == FALSE)
        BlitBitmapToPartyWindow(windowId, sSlotTilemap_Equal, 14, x, y, width, height);
    else
        BlitBitmapToPartyWindow(windowId, sSlotTilemap_EqualNoHP, 14, x, y, width, height);
}

static void DrawEmptySlot(u8 windowId)
{
    BlitBitmapToPartyWindow(windowId, sSlotTilemap_EqualEmpty, 14, 0, 0, 14, 5);
}
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE)
    {
        ...
        if (BW_SUMMARY_SCREEN)
        {
            if (gPartyMenu.layout == PARTY_LAYOUT_MULTI_FULL_PARTNER)
                ShowPokemonSummaryScreen_BW(SUMMARY_MODE_LOCK_MOVES, gParties[B_TRAINER_2], gPartyMenu.slotId, CalculatePartyCount(B_TRAINER_2) - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
            else if (gPartyMenu.layout == PARTY_LAYOUT_MULTI_FULL)
                ShowPokemonSummaryScreen_BW(SUMMARY_MODE_LOCK_MOVES, gParties[B_TRAINER_0], gPartyMenu.slotId, CalculatePartyCount(B_TRAINER_0) - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
            else
                ShowPokemonSummaryScreen_BW(SUMMARY_MODE_LOCK_MOVES, gParties[B_TRAINER_0], gPartyMenu.slotId, CalculatePartyCountOfSide(B_BATTLER_0) - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
        }
        else
        {
            if (gPartyMenu.layout == PARTY_LAYOUT_MULTI_FULL_PARTNER)
...
void CursorCb_MoveItemCallback(u8 taskId)
{
    ...
    case A_BUTTON:     // User hit A on a Pokemon
        ...
        if (ItemIsMail(GetMonData(&gParties[B_TRAINER_0][gPartyMenu.slotId2], MON_DATA_HELD_ITEM)))
        {
            PlaySE(SE_FAILURE);
            return;
        }
...
void ItemUseCB_BattleScript(u8 taskId, TaskFunc task)
{
    ...
    else
    {
        ...
        PlaySE(SE_SELECT);
        if (!IsItemFlute(gSpecialVar_ItemId) && GetItemPocket(gSpecialVar_ItemId) != POCKET_KEY_ITEMS)
            RemoveBagItem(gSpecialVar_ItemId, 1);
        ScheduleBgCopyTilemapToVram(2);
        gTasks[taskId].func = task;
    }
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task)
{
    ...
    if (NotUsingHPEVItemOn1HPMon(mon, item) == FALSE)
    {
        cannotUse = TRUE;
    }
    else
...
#define tState        data[0]
#define tMonId        data[1]
#define tOldBall      data[2]
#define tNewBall      data[3]
#define tOldFunc    4

void Task_Pokeball(u8 taskId)
{
    static const u8 askBallText[] = _("{STR_VAR_1} is in a\ndifferent Orb.\pDo you want to put it in\na {STR_VAR_2}?");
    static const u8 doneBallChanged[] = _("{STR_VAR_1} was put in the {STR_VAR_2}.{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
    ...
    case 3:
        ...
        StringExpandPlaceholders(gStringVar4, doneBallChanged);
        DisplayPartyMenuMessage(gStringVar4, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
        break;
