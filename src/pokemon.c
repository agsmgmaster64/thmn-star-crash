...
enum Ability GetMonAbility(struct Pokemon *mon)
{
    ...
    u8 cantRandomizeAbility = GetMonData(mon, MON_DATA_CANT_RANDOMIZE_ABILITY);
...
void PokemonToBattleMon(struct Pokemon *src, struct BattlePokemon *dst)
{
    ...
    dst->cantRandomizeAbility = GetMonData(src, MON_DATA_CANT_RANDOMIZE_ABILITY);
...
s32 CalculateFriendshipBonuses(struct Pokemon *mon, s32 modifier, enum HoldEffect itemHoldEffect)
{
    ...
    if (GetMonData(mon, MON_DATA_POKEBALL) == ITEM_LUXURY_ORB)
...
u16 GetBattleBGM(void)
{
    ...
    {
        u16 species = GetMonData(&gParties[B_TRAINER_1][0], MON_DATA_SPECIES);
        ...
        for (i = 0; i < count; i++)
        {
            ...
            else
            {
                ...
                if (rnd < chanceNotRare)
                    ...
                    SetMonData(&gParties[B_TRAINER_1][i], MON_DATA_HELD_ITEM, &gSpeciesInfo[species].itemSuperRare);
            }
        }
    }
}
...
enum Species GetFormChangeTargetSpeciesBoxMon(struct BoxPokemon *boxMon, enum FormChanges method)
{
    ...
    struct FormChangeContext ctx =
    {
        ...
        .ability = GetAbilityBySpecies(species, GetBoxMonData(boxMon, MON_DATA_ABILITY_NUM), GetBoxMonData(boxMon, MON_DATA_CANT_RANDOMIZE_ABILITY)),
