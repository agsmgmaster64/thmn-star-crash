...
static const u8 sText_UsedVar2WildLured[] = _("{PLAYER} used the\n{STR_VAR_2}.\pWild Puppets will be lured.{PAUSE_UNTIL_PRESS}");
static const u8 sText_UsedVar2WildRepelled[] = _("{PLAYER} used the\n{STR_VAR_2}.\pWild Puppets will be repelled.{PAUSE_UNTIL_PRESS}");
static const u8 sText_PlayedPokeFluteCatchy[] = _("Played the Toho Flute.\pNow, that's a catchy tune!{PAUSE_UNTIL_PRESS}");
static const u8 sText_PlayedPokeFlute[] = _("Played the Toho Flute.");
...
{
    enum ItemType type = GetItemType(gSpecialVar_ItemId) - 1;
...
static const struct YesNoFuncTable sUsePokevialYesNoPyramidBagFuncTable =
{
    .yesFunc = UsePokevialYes,
    .noFunc = CloseItemMessage,
};
...
void PokevialPrintDosesAndConfirmMessage(u32 currentDoses, bool32 isPlayerUsingRegisteredKeyItem, u8 taskId)
{
    ...
    if (currentDoses == 1)
    {
        StringExpandPlaceholders(gStringVar4, gText_PokevialHasOneDose);
    }
    else
    {
...
void ItemUseOutOfBattle_Pokevial(u8 taskId)
{
    ...
    CopyItemName(ITEM_TOHOVIAL, gStringVar1);
...
{
    InitRegionMapWithExitCB(REGIONMAP_TYPE_NORMAL, GetRegionMap(gMapHeader.regionMapSectionId), CB2_BagMenuFromStartMenu);
}
...
static void Task_UseTownMapFromField(u8 taskId)
{
    ...
    {
        CleanupOverworldWindowsAndTilemaps();
        InitRegionMapWithExitCB(REGIONMAP_TYPE_NORMAL, GetRegionMap(gMapHeader.regionMapSectionId), CB2_ReturnToField);
        DestroyTask(taskId);
    }
}
