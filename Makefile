...
CRY_BIN_DIR = sound/direct_sound_samples/cries
VOICE_BIN_DIR = sound/direct_sound_samples/voice_lines
TEST_SUBDIR = test
...
ifeq ($(SETUP_PREREQS),1)
  ...
  $(foreach line, $(shell $(MAKE) BUILD_NAME=$(BUILD_NAME) generated | sed "s/ /__SPACE__/g"), $(info $(subst __SPACE__, ,$(line))))
  ifneq ($(.SHELLSTATUS),0)
...
clean-assets:
	...
	rm -f $(CRY_BIN_DIR)/*.bin
	rm -f $(VOICE_BIN_DIR)/*.bin
