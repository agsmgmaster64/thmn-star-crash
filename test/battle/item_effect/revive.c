...
DOUBLE_BATTLE_TEST("Revive works for a partner in a double battle")
{
    GIVEN {
        ...
        TURN { MOVE(opponentRight, MOVE_MANA_BURST, target:playerLeft); MOVE(playerLeft, MOVE_CELEBRATE); } // Wynaut faints
        TURN { USE_ITEM(playerRight, ITEM_REVIVE, partyIndex: 0); MOVE(opponentRight, MOVE_MANA_BURST, target:playerRight); } // Wynaut gets revived, Wobb faints
