...
WILD_BATTLE_TEST("Higher leveled Pokemon give more exp", s32 exp)
{
    ...
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Level(20); }
...
WILD_BATTLE_TEST("Lucky Egg boosts gained exp points by 50%", s32 exp)
{
    ...
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Level(20); Item(item); }
...
WILD_BATTLE_TEST("Exp is scaled to player and opponent's levels", s32 exp)
{
    ...
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Level(level); }
...
WILD_BATTLE_TEST("Large exp gains are supported", s32 exp) // #1455
{
    ...
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Level(1); Item(ITEM_LUCKY_EGG); OTName("Test"); } // OT Name is different so it gets more exp as a traded mon
        OPPONENT(SPECIES_TECH_SHOU) { Level(level); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
        MESSAGE("Wobbuffet used Scratch!");
...
WILD_BATTLE_TEST("Exp Share(held) gives Experience to mons which did not participate in battle")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_NORMAL_KOSUZU) { Level(40); Item(item); }
        OPPONENT(SPECIES_NORMAL_DAIYOUSEI) { Level(10); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); }
    } SCENE {
        ...
        if (item == ITEM_EXP_SHARE)
            EXPECT_GT(GetMonData(&gPlayerParty[1], MON_DATA_EXP), gExperienceTables[gSpeciesInfo[SPECIES_NORMAL_KOSUZU].growthRate][40]);
        else
            EXPECT_EQ(GetMonData(&gPlayerParty[1], MON_DATA_EXP), gExperienceTables[gSpeciesInfo[SPECIES_NORMAL_KOSUZU].growthRate][40]);
    }
}
