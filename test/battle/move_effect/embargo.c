...
WILD_BATTLE_TEST("Embargo doesn't block held item effects that affect effort values")
{
    ...
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Item(ITEM_POWER_WEIGHT); }
        ...
        EXPECT_EQ(GetMonData(&gParties[B_TRAINER_0][0], MON_DATA_HP_EV), finalHPEVAmount);
    }
}
...
{
    GIVEN {
        PLAYER(SPECIES_CHIBI_YUUGI) { Speed(19); }
        OPPONENT(SPECIES_CHIBI_YUUGI) { Item(ITEM_MACHO_BRACE); Speed(20); }
...
//         finalFriendship = GetMonData(&gParties[B_TRAINER_0][0], MON_DATA_FRIENDSHIP);
...
SINGLE_BATTLE_TEST("Embargo doesn't block a held item's form-changing effect, but it does block its other effects", s16 damage)
{
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_MEADOW_PLATE].secondaryId == TYPE_NATURE);
    } WHEN {
