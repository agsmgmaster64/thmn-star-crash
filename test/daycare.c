...
#define STORE_IN_DAYCARE_AND_GET_EGG()                                          \
    StorePokemonInDaycare(&gParties[B_TRAINER_0][0], &gSaveBlock1Ptr->daycare.mons[0]);  \
    StorePokemonInDaycare(&gParties[B_TRAINER_0][0], &gSaveBlock1Ptr->daycare.mons[1]);  \
...
TEST("(Daycare) Pokémon generate Eggs of the lowest member of the evolutionary family")
{
    ...
    EXPECT_EQ(GetMonData(&gParties[B_TRAINER_0][0], MON_DATA_SPECIES), SPECIES_PICHU);
}
...
TEST("(Daycare) Pokémon offspring species is based off the mother's species")
{
    ...
    ZeroPlayerPartyMons();
    PARAMETRIZE { offspring = SPECIES_PLACEHOLD_KIKURI; RUN_OVERWORLD_SCRIPT(givemon SPECIES_PIKACHU, 100, gender=MON_MALE;   givemon SPECIES_CHIBI_YUUGENMAGAN, 100, gender=MON_FEMALE, item=ITEM_NONE;     ); }
    PARAMETRIZE { offspring = SPECIES_PICHU; RUN_OVERWORLD_SCRIPT(givemon SPECIES_PIKACHU, 100, gender=MON_FEMALE; givemon SPECIES_CHIBI_YUUGENMAGAN, 100, gender=MON_MALE;); }
    STORE_IN_DAYCARE_AND_GET_EGG();

    EXPECT_EQ(GetMonData(&gParties[B_TRAINER_0][0], MON_DATA_SPECIES), offspring);
}
...
TEST("(Daycare) Pokémon can breed with Ditto if they don't belong to the Ditto or No Eggs Discovered group")
{
    ...
    if (gSpeciesInfo[parentSpecies].eggGroups[0] != EGG_GROUP_NO_EGGS_DISCOVERED
     && gSpeciesInfo[parentSpecies].eggGroups[0] != EGG_GROUP_DITTO)
        EXPECT_NE(GetMonData(&gParties[B_TRAINER_0][0], MON_DATA_SPECIES), SPECIES_NONE);
    else
        EXPECT_EQ(GetMonData(&gParties[B_TRAINER_0][0], MON_DATA_SPECIES), SPECIES_NONE);
}
...
TEST("(Daycare) Pokémon with regional forms give the correct offspring")
{
    ...
    if (region == REGION_ALOLA) {
        PARAMETRIZE { offspring=SPECIES_GOLIATH_SANNI_DEFENSE;  species1=SPECIES_MEOWTH;        item1=ITEM_NONE;      species2=SPECIES_GOLIATH_SANNI_DEFENSE;  item2=ITEM_NONE;      }
        PARAMETRIZE { offspring=SPECIES_GOLIATH_SANNI_DEFENSE;  species1=SPECIES_MEOWTH;        item1=ITEM_NONE;      species2=SPECIES_GOLIATH_SANNI_DEFENSE;  item2=ITEM_EVERSTONE; }
        PARAMETRIZE { offspring=SPECIES_GOLIATH_SANNI_DEFENSE;  species1=SPECIES_MEOWTH;        item1=ITEM_NONE;      species2=SPECIES_MEOWTH_GALAR;  item2=ITEM_NONE;      }
        ...
        PARAMETRIZE { offspring=SPECIES_GOLIATH_SANNI_DEFENSE;  species1=SPECIES_DIGLETT;       item1=ITEM_NONE;      species2=SPECIES_MEOWTH_GALAR;  item2=ITEM_NONE;      }
        ...
        PARAMETRIZE { offspring=SPECIES_MEOWTH_GALAR;  species1=SPECIES_MEOWTH;        item1=ITEM_NONE;      species2=SPECIES_GOLIATH_SANNI_DEFENSE;  item2=ITEM_NONE;      }
        PARAMETRIZE { offspring=SPECIES_GOLIATH_SANNI_DEFENSE;  species1=SPECIES_MEOWTH;        item1=ITEM_NONE;      species2=SPECIES_GOLIATH_SANNI_DEFENSE;  item2=ITEM_EVERSTONE; }
        ...
        PARAMETRIZE { offspring=SPECIES_MEOWTH;        species1=SPECIES_MEOWTH;        item1=ITEM_NONE;      species2=SPECIES_GOLIATH_SANNI_DEFENSE,  item2=ITEM_NONE;      }
        PARAMETRIZE { offspring=SPECIES_GOLIATH_SANNI_DEFENSE;  species1=SPECIES_MEOWTH;        item1=ITEM_NONE;      species2=SPECIES_GOLIATH_SANNI_DEFENSE,  item2=ITEM_EVERSTONE; }
    ...
    if (region == REGION_HISUI) {
        ...
        PARAMETRIZE { offspring=SPECIES_SNEASEL;       species1=SPECIES_SNEASLER;      item1=ITEM_EVERSTONE; species2=SPECIES_CHIBI_MIMA,       item2=ITEM_EVERSTONE; }
    } else {
