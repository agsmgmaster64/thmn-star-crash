...
#define B_PRANKSTER_DARK_TYPES      GEN_6 // In Gen7+, Prankster-elevated status moves do not affect Dark-types.
...
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_PLAYER_AI     // If this flag is set, the player's Pokémon will be controlled by the AI in the next battle.
...
#define B_FLAG_TERA_ORB_CHARGED     FLAG_CAN_TERASTALIZE     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
#define B_FLAG_TERA_ORB_NO_COST     FLAG_TERA_ORB_NO_COST     // If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In SV, this occurs after an event with Terapagos.
...
#define B_FLAG_NO_WHITEOUT          FLAG_SCRIPTED_LOSS     // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically!
...
// Interface settings
#define B_FAST_INTRO_PKMN_TEXT              FALSE       // If set to TRUE, battle intro text prints at the same time as the animation of Pokémon, as opposing to waiting for the animation to end.
