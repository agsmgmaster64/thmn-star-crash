...
#define I_GEM_BOOST_POWER               GEN_5  // In Gen6+, the power multiplier of Gems is 1.3x instead of 1.5x.
...
#define VAR_LAST_REPEL_LURE_USED    VAR_LAST_USED_LURE_REPEL       // If this var has been assigned, the last Repel/Lure used will be saved and the player will be prompted with the vanilla repel YES/NO option, unless I_REPEL_LURE_MENU is set to TRUE.
#define I_REPEL_LURE_MENU           FALSE    // If set to TRUE, the player is able to choose which Repel/Lure to use once the previous one runs out. Cursor position is saved by VAR_LAST_REPEL_LURE_USED if not 0.

// Vs. Seeker
#define I_VS_SEEKER_CHARGING        FLAG_USE_VS_SEEKER                     // If this flag is assigned, the Vs. Seeker functionality will be enabled. When the player has the Vs. Seeker, Match Call rematch functions will stop working. Documentation for the Vs. Seeker can be found in docs/tutorials/vs_seeker.md.
...
#define I_ORAS_DOWSING_FLAG         FLAG_ORAS_DOWSE       // Replace 0 with an unused flag to enable the Dowsing Machine mechanic from ORAS.
